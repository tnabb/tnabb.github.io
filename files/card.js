//m_Card = [ id, position, name, description, ?, ... ]

m_Card = [
	[0, 0, "(no card)", 0, 0]
	, [1, 1, "* All Race Physical +20%", 0, 30, 20, 31, 20, 32, 20, 33, 20, 34, 20, 35, 20, 36, 20, 37, 20, 38, 20, 39, 20, 0]
	, [2, 1, "* All Element Physical +20%", 0, 40, 20, 41, 20, 42, 20, 43, 20, 44, 20, 45, 20, 46, 20, 47, 20, 48, 20, 49, 20, 0]
	, [3, 1, "* All Size Physical +15%, ATK +5", 0, 17, 5, 27, 15, 28, 15, 29, 15, 0]
	, [4, 1, "Fabre", 0, 3, 1, 13, 100, 0]
	, [5, 1, "Drops", 0, 5, 1, 8, 3, 0]
	, [6, 1, "Hornet", 0, 1, 1, 17, 3, 0]
	, [7, 1, "Lunatic", 0, 6, 1, 10, 1, 11, 1, 0]
	, [8, 1, "Female Thief Bug", 0, 2, 1, 9, 1, 0]
	, [9, 1, "Skeleton", 0, 17, 10, 131, 2, 0]
	, [10, 1, "Wolf", 0, 17, 15, 10, 1, 0]
	, [11, 1, "Andre", 0, 17, 20, 0]
	, [12, 1, "Peco Peco Egg", 0, 30, 20, 0]
	, [13, 1, "Hydra", 0, 37, 20, 0]
	, [14, 1, "Goblin", 0, 32, 20, 0]
	, [15, 1, "Caramel", 0, 34, 20, 0]
	, [16, 1, "Scorpion", 0, 33, 20, 0]
	, [17, 1, "Flora", 0, 35, 20, 0]
	, [18, 1, "Strouf", 0, 36, 20, 0]
	, [19, 1, "Earth Petite", 0, 39, 20, 0]
	, [20, 1, "Deviace", 0, 37, 7, 32, 7, 33, 7, 34, 7, 0]
	, [21, 1, "Drainliar", 0, 41, 20, 0]
	, [22, 1, "Santa Poring", 0, 47, 20, 0]
	, [23, 1, "Mandragora", 0, 44, 20, 0]
	, [24, 1, "Vadon", 0, 43, 20, 0]
	, [25, 1, "Anacondaq", 0, 45, 20, 0]
	, [26, 1, "Kaho", 0, 42, 20, 0]
	, [27, 1, "Orc Skeleton", 0, 46, 20, 0]
	, [28, 1, "Desert Wolf", 0, 27, 15, 17, 5, 0]
	, [29, 1, "Skeleton Worker", 0, 28, 15, 17, 5, 0]
	, [30, 1, "Minorous", 0, 29, 15, 17, 5, 0]
	, [31, 1, "Abysmal Knight", 0, 26, 25, 0]
	, [32, 1, "Drake", "Removes the size penalties of the weapon.<BR>(Certain weapon types have reduced attack power against certain enemy sizes).", 0]
	, [33, 1, "Plankton", 0, 17, 5, 135, 5, 0]
	, [34, 1, "Snake/Boa", 0, 17, 5, 130, 5, 0]
	, [35, 1, "Marina", 0, 17, 5, 132, 5, 0]
	, [36, 1, "Metaller", 0, 17, 5, 136, 5, 0]
	, [37, 1, "Magnolia", 0, 17, 5, 133, 5, 0]
	, [38, 1, "Zenorc", 0, 17, 10, 130, 4, 0]
	, [39, 1, "Golem", "Weapon cannot be broken", 17, 5, 0]
	, [40, 1, "Mummy", 0, 8, 20, 0]
	, [41, 1, "Soldier Skeleton", 0, 10, 9, 0]
	, [42, 1, "Doppelganger", 0, 12, 10, 0]
	, [43, 1, "Sidewinder", "Enable the use of [Double Attack] Lvl 1 with any weapons.<BR>If you learned a level higher than 1, you will use that level instead.", 0]
	, [44, 1, "Phreeoni", 0, 8, 100, 0]
	, [45, 1, "Baphomet", "All regular attacks become 3x3 splash damage like a Bowling Bash Effect Permanently.", 8, -10, 0]
	, [46, 1, "Hunter Fly", "When dealing ATK based damage, 3% chance you gain 15% of the damage inflicted on your target as HP.", 0]
	, [47, 1, "Dracula", "When dealing ATK based damage, 10% chance you gain 5% of the damage inflicted on your target as SP.", 0]
	, [48, 2, "Willow", 0, 14, 80, 0]
	, [49, 2, "Elder Willow", 0, 4, 2, 0]
	, [50, 2, "Nightmare", 0, 2, 1, 155, 100, 0]
	, [51, 2, "Deviruchi", 0, 1, 1, 154, 100, 0]
	, [52, 2, "Orc Hero", 0, 3, 3, 151, 100, 0]
	, [53, 3, "Thief Bug Egg", 0, 13, 400, 0]
	, [54, 3, "Andre Egg", 0, 15, 5, 0]
	, [55, 3, "Ambernite", 0, 18, 2, 0]
	, [56, 3, "Thara Frog", 0, 57, 30, 0]
	, [57, 3, "Orc Warrior", 0, 52, 30, 0]
	, [58, 3, "Bigfoot", 0, 54, 30, 0]
	, [59, 3, "Rafflesia", 0, 55, 30, 0]
	, [60, 3, "Sky Petite", 0, 59, 30, 0]
	, [61, 3, "Khalitzburg", 0, 56, 30, 0]
	, [62, 3, "Horn", 0, 78, 35, 0]
	, [63, 3, "Medusa", 0, 56, 15, 159, 100, 0]
	, [64, 4, "Poring", 0, 6, 2, 11, 1, 0]
	, [65, 4, "Picky", 0, 1, 1, 17, 10, 0]
	, [66, 4, "Super Picky", 0, 3, 1, 13, 100, 0]
	, [67, 4, "Thief Bug", 0, 2, 1, 0]
	, [68, 4, "Rocker", 0, 5, 1, 17, 5, 0]
	, [69, 4, "Baby Desert Wolf", 0, 4, 1, 0]
	, [70, 4, "Savage", 0, 3, 3, 0]
	, [71, 4, "Pupa", 0, 13, 700, 0]
	, [72, 4, "Roda Frog", 0, 13, 400, 14, 50, 0]
	, [73, 4, "Peco Peco", 0, 15, 10, 0]
	, [74, 4, "Steel Chonchon", "<hr><b>When worn with a [Pink Poo Poo Hat]:</b><br>Status effect [Freeze] resistance +100%", 18, 2, 64, 10, 0]
	, [75, 4, "Marc", 0, 152, 100, 61, 5, 0]
	, [76, 5, "Whisper", 0, 9, 20, 68, -50, 0]
	, [77, 5, "Raydric", 0, 60, 20, 0]
	, [78, 5, "Baphomet Jr.", 0, 2, 3, 10, 1, 0]
	, [79, 5, "Condor", 0, 9, 10, 0]
	, [80, 5, "Dustiness", 0, 9, 5, 64, 30, 0]
	, [81, 5, "Marse", 0, 9, 5, 61, 30, 0]
	, [82, 5, "Jakk", 0, 9, 5, 63, 30, 0]
	, [83, 5, "Hode", 0, 9, 5, 62, 30, 0]
	, [84, 5, "Marionette", 0, 9, 5, 68, 30, 0]
	, [85, 5, "Orc Zombie", 0, 9, 5, 69, 30, 0]
	, [86, 5, "Isis", 0, 9, 5, 67, 30, 0]
	, [87, 5, "Myst", 0, 9, 5, 65, 30, 0]
	, [88, 6, "Chonchon", 0, 9, 2, 2, 1, 0]
	, [89, 6, "Male Thief Bug", 0, 2, 2, 0]
	, [90, 6, "Matyr", 0, 2, 1, 15, 10, 0]
	, [91, 6, "Sohee", 0, 16, 15, 76, 3, 0]
	, [92, 6, "Verit", 0, 15, 8, 16, 8, 0]
	, [93, 6, "Zombie", 0, 75, 20, 0]
	, [94, 6, "Eggyra", 0, 76, 15, 0]
	, [95, 6, "Eddga", "Infinite [Endure] Effect, +1 MDEF Bonus. Doesn't work in GvG Maps.", 15, -25, 0]
	, [96, 7, "Spore", 0, 3, 2, 0]
	, [97, 7, "Kukre", 0, 2, 2, 0]
	, [98, 7, "Tarou", 0, 1, 2, 0]
	, [99, 7, "Wormtail", 0, 5, 2, 0]
	, [100, 7, "Zerom", 0, 5, 3, 0]
	, [101, 7, "Mantis", 0, 1, 3, 0]
	, [102, 7, "Yoyo", 0, 2, 1, 11, 5, 0]
	, [103, 7, "Kobold", 0, 1, 1, 10, 4, 0]
	, [104, 7, "Muka", 0, 75, 10, 0]
	, [105, 7, "Phen", "Skill casting cannot be interrupted (except in WoE).", 73, 25, 0]
	, [106, 1, "* Star Crumb", 0, 0]
	, [107, 1, "Archer Skeleton", 0, 25, 10, 0]
	, [108, 1, "Scorpion King", "<b>* WARNING: Not implemented in most servers.</b>", 49, 20, 0]
	, [109, 1, "Savage Babe", 0, 131, 5, 0]
	, [110, 1, "Familiar", 0, 17, 5, 134, 5, 0]
	, [111, 1, "Requiem", 0, 137, 5, 0]
	, [112, 1, "Andre Larva", "<b>* WARNING: Not implemented in most servers.</b>", 4, 1, 14, 10, 0]
	, [113, 2, "Stainer", 0, 18, 1, 156, 20, 0]
	, [114, 2, "Coco", 0, 18, 1, 155, 20, 0]
	, [115, 2, "Martin", 0, 18, 1, 154, 20, 0]
	, [116, 2, "Ghoul", 0, 18, 1, 150, 20, 0]
	, [117, 2, "Giearth", 0, 157, 100, 62, 15, 0]
	, [118, 2, "Marduk", 0, 156, 100, 0]
	, [119, 2, "Mistress", "Skills requiring gemstones to cast no longer require them.", 370, 25, 0]
	, [120, 2, "Pharaoh", "", 370, -30, 0]
	, [121, 3, "Soldier Andre", 0, 53, 30, 0]
	, [122, 3, "Anubis", 0, 58, 30, 0]
	, [123, 3, "Megalodon", 0, 18, 1, 152, 20, 0]
	, [124, 3, "Argos", 0, 18, 1, 159, 20, 0]
	, [125, 3, "Munak", 0, 18, 1, 62, 5, 159, 15, 0]
	, [126, 3, "Golden Thief Bug", "Immunity to all magic (including Heal, Buffs, Quagmire, etc).", 370, 100, 0]
	, [127, 3, "Maya", "50% chance to reflect magic spells targeted at you back to your enemy.", 0]
	, [128, 4, "Cornutus", "Armor becomes unbreakable.", 18, 1, 0]
	, [129, 6, "Moonlight Flower", "", 209, 25, 0]
	, [130, 7, "Osiris", "When revived, you recover full HP & SP", 0]
	, [131, 4, "Swordfish", 0, 18, 1, 198, 1, 0]
	, [132, 4, "Sandman", 0, 18, 1, 198, 2, 0]
	, [133, 4, "Pasana", 0, 18, 1, 198, 3, 0]
	, [134, 4, "Dokebi", 0, 18, 1, 198, 4, 0]
	, [135, 4, "Argiope", 0, 18, 1, 198, 5, 0]
	, [136, 4, "Angeling", 0, 198, 6, 0]
	, [137, 4, "Bathory", 0, 198, 7, 0]
	, [138, 4, "Ghostring", 0, 198, 8, 75, -25, 0]
	, [139, 4, "Evil Druid", 0, 4, 1, 18, 1, 198, 9, 0]
	, [140, 4, "Orc Lord", 0, 71, 30, 0]
	, [141, 5, "Frilldora", 0, 220, 19, 0]
	, [142, 7, "Beelzebub", 0, 73, -30, 0]
	, [143, 7, "Poison Spore", 0, 220, 20, 0]
	, [144, 7, "Marine Sphere", 0, 220, 21, 0]
	, [145, 7, "Vitata", "", 220, 5, 370, 25, 0]
	, [146, 7, "Smokie", 0, 220, 22, 0]
	, [147, 7, "Creamy", 0, 220, 17, 0]
	, [148, 7, "Poporing", 0, 220, 24, 0]
	, [149, 7, "Obeaune", 0, 220, 25, 0]
	, [150, 7, "Pirate Skel", 0, 220, 26, 0]
	, [151, 7, "Horong", 0, 220, 16, 0]
	, [152, 7, "Joker", 0, 220, 27, 0]
	, [153, 2, "* All Element -10%", "", 60, 10, 61, 10, 62, 10, 63, 10, 64, 10, 65, 10, 66, 10, 67, 10, 68, 10, 69, 10, 0]
	, [154, 3, "* All Race -30%", 0, 50, 30, 51, 30, 52, 30, 53, 30, 54, 30, 55, 30, 56, 30, 57, 30, 58, 30, 59, 30, 0]
	, [155, 3, "* All Element -20% & +5%", 0, 60, 20, 61, 20, 62, 20, 63, 20, 64, 20, 65, 20, 66, 20, 67, 20, 68, 20, 69, 20, 40, 5, 41, 5, 42, 5, 43, 5, 44, 5, 45, 5, 46, 5, 47, 5, 48, 5, 49, 5, 0]
	, [156, 1, "* CRIT Dmg +10%, CRIT +7", 0, 70, 10, 110, 7, 111, 7, 112, 7, 113, 7, 114, 7, 115, 7, 116, 7, 117, 7, 118, 7, 119, 7, 0]
	, [157, 1, "Metaling", 0, 221, 54, 0]
	, [158, 1, "Eremes Guile", 0, 117, 10, 0]
	, [159, 1, "Howard Alt-Eisen", 0, 8, 30, 12, -5, 0]
	, [160, 1, "Cecil Damon", 0, 12, 5, 8, -30, 0]
	, [161, 1, "Mastersmith", "When performing an ATK based attack, 10% chance to break your target's weapon.<br>When performing an ATK based attack, 7% chance to break your target's armor.", 0]
	, [162, 1, "Sniper", "When performing an ATK based attack, 5% chance to absorb 20% of the damage inflicted on your target as HP. HP Recovery Rate -100%", 75, -100, 0]
	, [163, 1, "Breeze", 0, 17, 5, 138, 5, 0]
	, [164, 1, "Lady Solace", "If equipped by a [Priest] class job:<BR>When performing an ATK based attack, 2% chance to cast [Grand Cross] Lvl 5", 0]
	, [165, 1, "Maero of Thanatos", 0, 17, 5, 221, 55, 0]
	, [166, 1, "Memory of Thanatos", "Consumes 10 SP on each ATK based attack.", 23, 1, 18, -30, 9, -30, 0]
	, [167, 1, "Hill Wind", "When unequipping the weapon with this card, consumes 50 SP.", 5057, 5, 5126, 5, 5127, 5, 0]
	, [168, 1, "Laurell Weinder", "When unequipping the weapon with this card, consumes 50 SP.", 5130, 3, 5131, 3, 0]
	, [169, 1, "Death Word", "When unequipping the weapon with this card, consumes 50 SP.", 5046, 5, 5047, 5, 5277, 5, 0]
	, [170, 1, "Red Ferus", "When unequipping the weapon with this card, consumes 50 SP.", 5122, 5, 5125, 5, 0]
	, [171, 1, "Pitman", "When unequipping the weapon with this card, consumes 50 SP.", 5132, 5, 5133, 5, -5319, 5, 0]
	, [172, 3, "Teddy Bear", 0, 51, 30, 0]
	, [173, 3, "Despero of Thanatos", "<b>For each refine level:</b> INT +1", 4, -6, 0]
	, [174, 2, "Ungoliant", 0, 75, 10, 158, 100, 0]
	, [175, 2, "Holden", 0, 6, 2, 0]
	, [176, 2, "Gemini-S58", "<b>[Base AGI >= 90]</b> [Silence] and [Stun] status effects resistance +30%<BR><b>[Base VIT >= 80]</b> [Stone Curse] and [Sleep] status effects resistance +50%", 0]
	, [177, 2, "Kathryne Keyron", "<b>For each refine level:</b> Casting Time -1%<BR><b>[Refine Rate 9~10]</b> MATK +2%", 0]
	, [178, 2, "Lord Knight", "Enables use of [" + skillName(258) + "] Lvl 1", 15, -50, 0]
	, [179, 2, "Blue Acidus", "<b>[Refine Rate 0~4]</b> MaxSP +40, SP Recovery Rate +5%", 14, 40, 0]
	, [180, 2, "Seyren Windsor", "<b>For each refine level:</b> STR +1", 1, -6, 0]
	, [181, 4, "Waste Stove", 0, 4, 1, 17, 5, 0]
	, [182, 4, "Venomous", "When receiving ATK based damage, 30% chance to cause [Poison] status effect on both yourself and your attacker.", 0]
	, [183, 4, "Porcellio", 0, 17, 25, 18, -5, 0]
	, [184, 4, "Mineral", 0, 17, -25, 18, 3, 0]
	, [185, 4, "Obsidian", "<b>For every 18 base DEX:</b> VIT +1", 0]
	, [186, 4, "Remover", "<b>For each refine level:</b> MaxHP -40", 13, 800, 0]
	, [187, 4, "Egnigem Cenia", "<b>For every 18 base INT:</b> STR +1", 0]
	, [188, 7, "Armeyer Dinze", "5% chance to obtain a [Clam Soup] (VIT +3) item when killing a [" + v_Race[5] + "] race monster.", 0]
	, [189, 4, "Venatu", "<b>For every 18 base AGI:</b> LUK +1", 0]
	, [190, 4, "Archdam", 0, 17, 10, 73, 20, 0]
	, [191, 4, "Ancient Mimic", "<b>For every 18 base LUK:</b> AGI +1", 0]
	, [192, 4, "Red Novus", "When receiving an ATK based attack, 30% chance to cause [Confusion] status effect on both yourself and your attacker.", 0]
	, [193, 4, "Yellow Novus", 0, 13, 500, 75, 10, 0]
	, [194, 4, "Alicel", 0, 9, 10, 18, -5, 0]
	, [195, 4, "Skogul", "When receiving an ATK based attack, 30% chance to cause [Bleeding] status effect on both yourself and your attacker.", 0]
	, [196, 4, "Mistress of Shelter", "<b>For every 18 base STR:</b> INT +1", 0]
	, [197, 4, "Observation", "<b>For every 18 base VIT:</b> DEX +1", 0]
	, [198, 4, "Dimik", "<b>For each refine level:</b> VIT +1", 3, -5, 0]
	, [199, 4, "Frus", "(refine level * 2)% chance to reflect magic spells targeted at you back to your enemy.<BR>If equipped by a [Mage] class job: MDEF +3", 0]
	, [200, 10, "0", 0, 0]
	, [201, 10, "* Element Stone (Water)", 0, 0]
	, [202, 10, "* Element Stone (Earth)", 0, 0]
	, [203, 10, "* Element Stone (Fire)", 0, 0]
	, [204, 10, "* Element Stone (Wind)", 0, 0]
	, [205, 10, "Unused", 0, 0]
	, [206, 10, "Unused", 0, 0]
	, [207, 10, "Unused", 0, 0]
	, [208, 10, "Unused", 0, 0]
	, [209, 10, "Unused", 0, 0]
	, [210, 10, "Unused", 0, 0]
	, [211, 10, "Unused", 0, 0]
	, [212, 7, "Quve", "If equipped by a [Novice] or [Super Novice] job: When receiving ATK based damage, 10% chance to cast [Increase AGI] Lvl 1 on yourself.", 0]
	, [213, 2, "Gibbet", "<b>[Refine Rate 0~5]</b> MDEF +5", 0]
	, [214, 4, "Disguise", "When receiving ATK based damage, 3% chance to cause [Silence] status effect on your attacker.<BR>[Base VIT >= 77] 9% chance instead.", 0]
	, [215, 1, "Dullahan", 0, 70, 10, 119, 7, 0]
	, [216, 7, "Hylozoist", "When performing an ATK based damage, 1% chance to polymorph the target.", 0]
	, [217, 1, "Bloody Murderer", 0, 70, 10, 114, 7, 0]
	, [218, 7, "Lude", "If equipped by a [Novice] or [Super Novice] job: When receiving ATK based damage, 20% chance to cast [Endure] Lvl 1 on yourself.", 0]
	, [219, 1, "Lord of the Dead", "When performing a Melee ATK based attack, 5% chance to cause [Stun] status effect on your target.<br>When performing a Melee ATK based attack, 5% chance to cause [Curse] status effect on your target.<br>When performing a Melee ATK based attack, 5% chance to cause [Silence] status effect on your target.<br>When performing a Melee ATK based attack, 5% chance to cause [Poison] status effect on your target.<br>When performing a Melee ATK based attack, 5% chance to cause [Bleeding] status effect on your target.<br>When performing an ATK based attack, 0.1% chance to cause [Coma] status effect on your target.", 0]
	, [220, 4, "Loli Ruri", "When receiving ATK based damage, 5% chance to cast [Heal] Lvl 3 on yourself.", 0]
	, [221, 2, "Arch Angeling", "[Base LUK >= 77] HP and SP Recovery Rate +100%", 13, 300, 0]
	, [222, 3, "Arclouze", "<b>[Refine Rate 0~5]</b> DEF +2, MDEF +3", 0]
	, [223, 1, "Assaulter", 0, 70, 10, 117, 7, 0]
	, [224, 4, "Anolian", "If you learned [" + skillName(42) + "] Lvl 10, you will cast Lvl 10 instead.", 221, 56, 0]
	, [225, 4, "Apocalypse", "<b>[Refine Rate 9~10]</b> MaxHP +800", 3, 2, 0]
	, [226, 7, "Jing Guai / Li Me Mang Ryang", "1% chance to obtain a [Box of Sunlight] item when killing an [" + v_Race[8] + "] race monster.", 0]
	, [227, 6, "Am Mut", 0, 127, 10, 57, -20, 0]
	, [228, 6, "Amon Ra", "[Base INT >= 99] The chance to cast [Kyrie Eleison] increases to 10%", 7, 1, 221, 57, 0]
	, [229, 6, "Alarm", 0, 3, 1, 13, 300, 0]
	, [230, 7, "Alligator", 0, 78, 5, 0]
	, [231, 3, "Alice", 0, 77, 40, 79, -40, 0]
	, [232, 4, "Antonio", 0, 221, 58, 0]
	, [233, 1, "The Paper", "Consumes 1 SP on each ATK based attack.", 70, 20, 0]
	, [234, 2, "Incubus", 0, 14, 150, 4, -3, 76, -20, 0]
	, [235, 1, "Injustice", 0, 221, 59, 0]
	, [236, 7, "Violy", "If you learned [" + skillName(201) + "] Lvl 5, you will cast Lvl 5 instead.", 221, 60, 0]
	, [237, 7, "Wind Ghost", "If you learned [Jupitel Thunder] Lvl 10, you will cast Lvl 10 instead.", 221, 61, 0]
	, [238, 2, "Wootan Shooter", 0, 18, 1, 157, 20, 0]
	, [239, 2, "Wootan Fighter", 0, 18, 1, 158, 20, 0]
	, [240, 4, "Wooden Golem", 0, 75, 30, 18, 1, 0]
	, [241, 3, "Executioner", 0, 18, 1, 192, 25, 0]
	, [242, 6, "Explosion", 0, 129, 10, 59, -20, 0]
	, [243, 5, "Eclipse", 0, 3, 1, 0]
	, [244, 1, "Elder", "Increases ATK based damage against [Guardian] type monsters by 40%<BR>[does not work on WoE: SE Guardians]", 1063, 40, 1064, 40, 1065, 40, 1575, 40, 1576, 40, 0]
	, [245, 3, "Ancient Mummy", 0, 221, 62, 0]
	, [246, 6, "Ancient Worm", 0, 126, 10, 56, -20, 0]
	, [247, 7, "Owl Duke", 0, 221, 63, 0]
	, [248, 7, "Owl Baron", 0, 221, 64, 0]
	, [249, 3, "Tirfing/Ogretooth", 0, 18, 1, 191, 25, 0]
	, [250, 7, "Orc Archer", "1% chance to obtain a [Box of Panting] item when killing a [" + v_Race[7] + "] race monster.", 0]
	, [251, 7, "Poison Toad", "Causes [Envenom] skill to push your target 5 cells back.", 221, 65, 0]
	, [252, 1, "Orc Lady", "<b>Does not work on [Orc Hero] neither [Orc Lord].</b>", 83, 30, 0]
	, [253, 1, "Fur Seal", "If equipped by an [Acolyte] class job: CRIT +9 against [" + v_Race[1] + "] and [" + v_Race[6] + "] race monsters.", 8, 10, 9, 3, 0]
	, [254, 1, "Shellfish", "Increases ATK based damage against [Crab] by 30%", 1072, 30, 17, 5, 0]
	, [255, 1, "Incantation Samurai", "Disables your natural HP Recovery Rate and consumes 666 HP every 10 seconds.<BR>Consumes 999 HP when unequipping.", 22, 1, 0]
	, [256, 7, "Gargoyle", "1% chance to obtain a [Box of Thunder] item when killing an [" + v_Race[4] + "] race monster.", 0]
	, [257, 6, "Gajomart", 0, 123, 10, 53, -20, 0]
	, [258, 5, "Kapha", "<b>[Refine Rate 0~5]</b> MDEF +8<hr><b>When compounded into an [Elegant Manteau]:</b><br>Value lowered to MDEF +1", 0]
	, [259, 1, "Crab", "Increases ATK based damage against [Aster] by 30%", 1240, 30, 17, 5, 0]
	, [260, 7, "Shinobi", "When receiving ATK based damage, 10% chance to cast [Cloaking] Lvl 5 on yourself.", 2, 1, 0]
	, [261, 4, "Karakasa", "When receiving ATK based damage, 3% chance to cause [Confusion] status effect on your attacker.<BR>[Base STR >= 77] 9% chance instead.", 0]
	, [262, 7, "Galapago", "3% chance to obtain an [Apple Juice], [Banana Juice] or [Carrot Juice] items when killing an [" + v_Race[4] + "] race monster.<BR>[Apple Juice], [Banana Juice] and [Carrot Juice] items recover +50%", 0]
	, [263, 1, "Gig", "Recover 5 SP when killing an [" + v_Race[4] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [264, 4, "Chimera", "When receiving ATK based damage, 3% chance to cause [Poison] status effect on your attacker.<BR>If equipped by an [Assassin] class job: 9% chance instead.", 0]
	, [265, 1, "Caterpillar", "Recover 5 SP when killing a [" + v_Race[3] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [266, 6, "Cat O' Nine Tails", "5% chance to reflect magic spells targeted at you back to your enemy.", 19, 3, 0]
	, [267, 5, "Giant Whisper", "[Base STR >= 80] ATK +20<BR>[Base VIT >= 80] MaxHP +3%<BR>[Base LUK >= 80] CRIT +3", 9, 10, 0]
	, [268, 4, "Killer Mantis", "When receiving ATK based damage, 6% chance to cause [Bleeding] status effect on your attacker.", 0]
	, [269, 6, "Christmas Cookie", 0, 128, 10, 58, -20, 0]
	, [270, 7, "Cookie", 0, 6, 2, 5037, 10, 0]
	, [271, 5, "Ninetails", "<b>[Refine Rate 9~10]</b> FLEE +20", 2, 2, 0]
	, [272, 3, "Mi Gao", "-50% damage from [Guardian] type monsters.<BR>(does not work on WoE:SE Guardians as of Nov. 2008)", 3063, 50, 3064, 50, 3065, 50, 0]
	, [273, 2, "Cramp", "1% chance to obtain 1 to 500 zeny when killing a monster.", 0]
	, [274, 2, "Grand Peco", 0, 221, 66, 0]
	, [275, 4, "Creamy Fear", "When receiving ATK based damage, 6% chance to cause [Confusion] status effect on your attacker.", 0]
	, [276, 4, "Grizzly", "When receiving ATK based damage, 3% chance to cause [Blind] status effect on your attacker.", 0]
	, [277, 1, "Gryphon", "If equipped by a [Swordman] class job: When performing an ATK based damage, 1% chance to cast [Bowling Bash] Lvl 5 on your target.", 9, 2, 10, 7, 0]
	, [278, 6, "Gullinbursti", 0, 125, 10, 55, -20, 0]
	, [279, 1, "Cruiser", 0, 70, 10, 112, 7, 0]
	, [280, 4, "Clock", "If the user has learned Lvl 10, that level is used instead.", 221, 67, 0]
	, [281, 4, "Baby Leopard", "If equipped by a [Merchant] class job: Armor becomes unbreakable.", 6, 3, 0]
	, [282, 4, "Aliza", "When receiving ATK based damage, 5% chance to cast [" + skillName(539) + "] on your attacker.<BR>If equipped by a [Dancer] class job: 10% chance instead.", 0]
	, [283, 4, "Goat", "<b>[Refine Rate 0~5]</b> DEF +2, MDEF +5", 0]
	, [284, 2, "Evil Snake Lord", 0, 4, 3, 154, 100, 153, 100, 0]
	, [285, 4, "Majoruros", "When receiving ATK based damage, 6% chance to cause [Stun] on your attacker.", 0]
	, [286, 1, "Goblin Archer", 0, 70, 10, 111, 7, 0]
	, [287, 1, "Goblin Steamrider", 0, 70, 10, 110, 7, 0]
	, [288, 1, "Goblin Leader", 0, 81, 30, 0]
	, [289, 1, "Kobold Archer", 0, 70, 10, 113, 7, 0]
	, [290, 1, "Kobold Leader", 0, 82, 30, 0]
	, [291, 4, "Succubus", 0, 13, 1000, 3, -3, 75, -20, 0]
	, [292, 7, "Tengu", "Chance to obtain a healing type item when killing a monster (not implemented).", 0]
	, [293, 4, "Sasquatch", "When receiving ATK based damage, 6% chance to cause [Freeze] status effect on your attacker.", 0]
	, [294, 5, "Vagabond Wolf", 0, 1, 1, 0]
	, [295, 5, "Wanderer", "If equipped by a [Thief] class job: FLEE +20", 221, 68, 0]
	, [296, 1, "Tri Joint", "Recover 5 SP when killing a [" + v_Race[0] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [297, 7, "Sea-Otter", "3% chance to obtain a [Raw Fish] or [Sushi] items when killing a [" + v_Race[5] + "] race monster.<BR>+50% HP restoration with those items.", 0]
	, [298, 2, "Carat", "<b>[Refine Rate 9~10]</b> MaxSP +150", 4, 2, 0]
	, [299, 4, "Kraben", "When receiving ATK based damage, 6% chance to cause [Blind] status effect on your attacker.", 0]
	, [300, 6, "Skeleton General", 0, 124, 10, 54, -20, 0]
	, [301, 4, "Geographer", "If the user has learned level 10, that level is used instead.", 221, 69, 0]
	, [302, 4, "Giant Spider", "When receiving ATK based damage, 6% chance to cause [Poison] status effect on your attacker.", 0]
	, [303, 2, "Giant Hornet", "1% chance to obtain a [Wind of Verdure] item when killing an [" + v_Race[4] + "] race monster.", 64, 10, 0]
	, [304, 6, "Firelock Soldier", "<b>[Refine Rate 9~10]</b> MaxHP +10% and MaxSP +10%", 1, 2, 0]
	, [305, 1, "Baby Garm / Hatii", 0, 221, 70, 0]
	, [306, 3, "Zealotus / Zherlthsh", 0, 5207, 10, 5199, 10, 6, 2, 0]
	, [307, 3, "Enchanted Peach Tree", "If the user has learned level 10, that level is used instead.", 221, 172, 0]
	, [308, 4, "Skel Prisoner", "When receiving ATK based damage, 3% chance to cause [Sleep] status effect on your attacker.", 0]
	, [309, 2, "Stalactic Golem", 0, 18, 1, 151, 20, 0]
	, [310, 3, "Sting", "<b>[Refine Rate 9~10]</b> MDEF +5", 18, 2, 0]
	, [311, 1, "Dark Priest", "When performing an ATK based attack, 5% chance to decrease your target's SP by 10%<BR>If equipped by a [Sage] class job: Recover 1 SP when dealing ATK based damage.", 0]
	, [312, 7, "Stem Worm", "1% chance to obtain a [Box of Storms] item when killing a [" + v_Race[2] + "] race monster.", 0]
	, [313, 1, "Stormy Knight", 0, 132, 20, 221, 71, 0]
	, [314, 7, "Spring Rabbit", "2% chance to obtain a [Meat] or [Monster's Feed] item when killing a [" + v_Race[2] + "] race monster.<BR>+50% HP restoration with [Meat] and [Monster's Feed] items.", 0]
	, [315, 7, "Sleeper", "1% chance to obtain a [Box of Drowsiness] item when killing a [" + v_Race[5] + "] race monster.", 0]
	, [316, 7, "Sage Worm", "0.3% chance to obtain a [Blue/Red/Yellow Gemstone] item when killing a monster.", 0]
	, [317, 4, "Solider", 0, 18, 2, 19, 2, 0]
	, [318, 6, "Zombie Prisoner", 0, 121, 10, 51, -20, 0]
	, [319, 1, "Zombie Master", "Recover 5 SP when killing an [" + v_Race[1] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [320, 2, "Dark Illusion", 0, 73, -10, 15, -10, 16, -10, 0]
	, [321, 4, "Dark Frame", "When receiving ATK based damage, 6% chance to cause [Stone Curse] status effect on your attacker.", 0]
	, [322, 6, "Dark Lord", 0, 221, 72, 0]
	, [323, 1, "Turtle General", 0, 80, 20, 221, 73, 0]
	, [324, 7, "Dumpling Kid", "3% chance to obtain a [Candy] item when killing a [" + v_Race[7] + "] race monster.<br>1% chance to obtain a [Candy Cane] item when killing a [" + v_Race[7] + "] race monster.<BR>+50% HP restoration with [Candy] and [Candy Cane] items.", 0]
	, [325, 1, "Chepet", "(on your enemy, not yourself)", 221, 74, 0]
	, [326, 1, "Zipper Bear", "Consumes 1 SP on each ATK based attack.<BR>If equipped by a [Merchant] class job: Weapon becomes unbreakable.", 17, 30, 0]
	, [327, 5, "Choco", 0, 9, 10, 11, 5, 0]
	, [328, 1, "Mobster", "If equipped by a [Thief] class job: CRIT +4", 70, 15, 0]
	, [329, 1, "Diabolic", "Recover 5 SP when killing a [" + v_Race[6] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [330, 4, "Demon Fungus", "When receiving ATK based damage, 6% chance to cause [Sleep] status effect on your attacker.", 0]
	, [331, 4, "Wraith Dead", "When receiving ATK based damage, 6% chance to cause [Curse] status effect on your attacker.", 0]
	, [332, 2, "White Lady / Bacsojin", "", 99, 30, 370, 15, 0]
	, [333, 4, "Sky Deleter", "Recover 100 HP when killing a monster with ATK based damage.", 0]
	, [334, 4, "Earth Deleter", "Recover 10 SP when killing a monster with ATK based damage.<BR>Consumes 100 SP when the armor is unequipped.", 0]
	, [335, 7, "Greatest General", "0.2% chance to cast [Summon Spirit Sphere] Lvl 5 when performing an ATK based attack.<BR>If equipped by an [Acolyte] class job: 2% chance instead.", 0]
	, [336, 7, "Taoist Hermit", "1% chance to obtain a [Box of Gloom] item when killing a [" + v_Race[3] + "] race monster.", 0]
	, [337, 2, "Evil Nymph", 0, 4, 1, 14, 50, 0]
	, [338, 5, "Toad", 0, 11, 1, 0]
	, [339, 2, "Clock Tower Manager", 0, 73, -5, 4, 1, 0]
	, [340, 5, "Dragon Tail", 0, 5040, 5, 5041, 5, 2, 1, 9, 10, 0]
	, [341, 5, "Dragon Fly", 0, 2, 1, 0]
	, [342, 2, "Dryad", "1% chance to obtain a [Green Live] item when killing a [" + v_Race[3] + "] race monster.", 62, 10, 0]
	, [343, 1, "Driller", "Recover 5 SP when killing a [" + v_Race[9] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [344, 4, "Nightmare Terror", "When receiving ATK based damage, 3% chance to cause [Curse] status effect on your attacker.", 0]
	, [345, 1, "Nereid", "Recover 5 SP when killing a [" + v_Race[2] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [346, 5, "Harpy", 0, 60, 15, 5046, 5, 0]
	, [347, 2, "Permeter", 0, 67, 15, 69, 15, 0]
	, [348, 3, "High Orc", 0, 18, 1, 71, 5, 0]
	, [349, 4, "Garm / Hatii", "When receiving ATK based damage, 50% chance to cause [Freeze] status effect on your attacker.", 0]
	, [350, 7, "Bloody Butterfly", "Skill casting cannot be interrupted (except in WoE).", 73, 30, 5053, 5, 0]
	, [351, 6, "Merman", 0, 75, 10, 76, 10, 0]
	, [352, 5, "Punk", "If the user has learned Lvl 5, that level is used instead.", 221, 75, 0]
	, [353, 1, "Panzer Goblin", 0, 70, 10, 116, 7, 0]
	, [354, 7, "Heater", "<b>[Swordman Class]</b><br>Perfect Dodge +3", 10, 3, 0]
	, [355, 1, "Beetle King", "Recover 5 SP when killing a [" + v_Race[5] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [356, 1, "Aster", "+30% damage on [Shellfish]", 1059, 30, 17, 5, 0]
	, [357, 2, "Kiel D-01", 0, 74, 30, 0]
	, [358, 3, "Parasite", "", 60, 5, 18, 1, 0]
	, [359, 1, "Fake Angel", "Recover 5 SP when killing an [" + v_Race[8] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [360, 1, "Phendark", "Recover 5 SP when killing a [" + v_Race[7] + "] race monster with ATK based damage.<BR>Consumes 5 SP when the weapon is unequipped.", 0]
	, [361, 1, "Bloody Knight", 0, 221, 76, 0]
	, [362, 6, "Freezer", "<b>[Refine Rate 9~10]</b> +10% damage with [Bash].", 13, 300, 0]
	, [363, 1, "Valkyrie Randgris", "Weapon becomes unbreakable.", 80, 10, 221, 77, 0]
	, [364, 4, "Brilight", "When receiving ATK based damage, 6% chance to cause [Silence] status effect on your attacker.", 0]
	, [365, 7, "Blazzer", "Chance to obtain a food type item when killing a monster.<br><b>LIST OF FOOD ITEMS DROPPED:</b> 1% Banana, 1% Sweet Potato, 0.5% Ice Cream, 0.5% Fresh Fish, 0.5% Cheese, 0.5% Sushi, 0.5% Bread, 0.5% Mojji, 0.5% Pumpkin, 0.3% Apple Juice, 0.3% Banana Juice, 0.3% Grape Juice, 0.3% Carrot Juice, 0.3% Well Baked Cookie, 0.1% STR+2 dish, 0.1% AGI+2 dish, 0.1% VIT+3 dish, 0.1% DEX+2 dish, 0.1% LUK+3 dish, 0.1% INT+2 dish, 0.05% Royal Jelly.", 0]
	, [366, 1, "Stone Shooter", 0, 8, 10, 17, 10, 0]
	, [367, 4, "Pest", "When receiving ATK based damage, 3% chance to cause [Stone Curse] status effect on your attacker.<BR>[Base INT >= 77] 9% chance instead.", 0]
	, [368, 3, "Penomena", 0, 50, 30, 0]
	, [369, 5, "Vocal", 0, 19, 3, 0]
	, [370, 7, "Bongun", "You receive double damage from [Munak] monster.<BR>Causes [Bash] skill to push your target 5 cells back.", 221, 78, 3302, -100, 0]
	, [371, 7, "Marin", "20% chance to obtain a [Jellopy] item when killing a monster.<br>0.1% chance to obtain a [Large Jellopy] item when killing a monster.", 0]
	, [372, 5, "Mastering", 0, 6, 1, 0]
	, [373, 10, 0, 0, 0]
	, [374, 2, "Maya Purple", "Hidden monsters and players become visible.", 0]
	, [375, 3, "Mysteltainn", 0, 18, 1, 190, 25, 0]
	, [376, 2, "Myst Case", "0.3% chance to obtain a [Gift Box] item when killing a monster.", 0]
	, [377, 6, "Mini Demon", 0, 122, 10, 52, -20, 0]
	, [378, 7, "Mimic", "0.1% chance to obtain an [Old Blue Box] item when killing a monster.", 0]
	, [379, 6, "Miyabi Doll", 0, 16, 10, 5055, 5, 0]
	, [380, 1, "Mutant Dragonoid", "If the user has learned level 10, that level is used instead.", 17, 15, 221, 79, 0]
	, [381, 6, "Megalith", "<b>[Refine Rate 0~5]</b> MDEF +7", 0]
	, [382, 1, "Lava Golem", 0, 84, 30, 0]
	, [383, 2, "Rideword", "If equipped by an [Acolyte] class job: MDEF +1, INT +1", 4, 1, 0]
	, [384, 4, "Rybio", "When receiving ATK based damage, 3% chance to cause [Stun] on your attacker.<BR>[Base DEX >= 77] 9% chance instead.", 0]
	, [385, 7, "Wraith", "1% chance to obtain a [Giggling Box] item when killing an [" + v_Race[1] + "] race monster.", 0]
	, [386, 7, "Raydric Archer", "1% chance to obtain a [Box of Resentment] item when killing a [" + v_Race[6] + "] race monster.", 0]
	, [387, 2, "Leib Olmai", "1% chance to obtain a [Red Blood] item when killing a [" + v_Race[2] + "] race monster.", 63, 10, 0]
	, [388, 6, "Raggler", 0, 1, 1, 3, 1, 0]
	, [389, 7, "Hermit Plant", "3% chance to obtain a [Red Herb] item when killing a [" + v_Race[3] + "] race monster.<BR>2% chance to obtain a [Yellow Herb] item when killing a [" + v_Race[3] + "] race monster.<BR>1% chance to obtain a [White Herb] item when killing a [" + v_Race[3] + "] race monster.<BR>+50% HP restoration with [Red Herb], [Yellow Herb], and [White Herb] items.", 0]
	, [390, 1, "Rotar Zairo", 0, 70, 10, 115, 7, 0]
	, [391, 6, "Wild Rose", "<b>[Thief class]</b> Perfect Dodge +5", 2, 1, 0]
	, [392, 4, "Tao Gunka", "DEF -50<BR>MDEF -50", 15, 100, 0]
	, [393, 5, "Yao Jun", 0, 9, 15, 10, 1, 0]
	, [394, 5, "Deviling", 0, 60, 50, 61, -50, 62, -50, 63, -50, 64, -50, 65, -50, 66, -50, 67, -50, 68, -50, 69, -50, 0]
	, [395, 7, "Zhu Po Long", 0, 2, 1, 10, 3, 0]
	, [396, 3, "Tamruan", 0, 18, 2, 5158, 10, 5159, 10, 5384, 10, 0]
	, [397, 2, "Leaf Cat", "1% chance to obtain a [Crystal Blue] item when killing a [" + v_Race[5] + "] race monster.", 61, 10, 0]
	, [398, 1, "Mao Guai / Civil Servant", 0, 48, 20, 0]
	, [399, 6, "Iron Fist", 0, 120, 10, 50, -20, 0]
	, [400, 5, "Noxious", 0, 60, 10, 78, 10, 0]
	, [401, 5, "Kavach Icarus", "<b>[Refine Rate 0~4]</b> FLEE +20, Perfect Dodge +1<br><b>[Refine Rate 5~10]</b> Instead FLEE +10", 0]
	, [402, 5, "Chung E", "<b>For each refine level:</b> LUK +1, CRIT +1", 6, -5, 0]
	, [403, 5, "Orc Baby", "<b>[Refine Rate 9~10]</b> Additional FLEE +5 and -5% damage from [" + v_Element[0] + "] element.", 60, 10, 9, 10, 0]
	, [404, 5, "Assassin Cross", 0, 220, 28, 0]
	, [405, 5, "Aliot", "If equipped by a [Swordman], [Thief], or [Merchant] class job: MaxHP +5%, STR +2<BR>If equipped by an [Acolyte], [Mage], or [Archer] class job: MaxSP +5%, INT +2", 0]
	, [406, 6, "Odium of Thanatos", "<b>For each refine level:</b> AGI +1", 2, -5, 0]
	, [407, 6, "Gold Acidus", "<b>[Refine Rate 0~4]</b> Additional MaxHP +4%, MaxSP +4%, HP Recovery Rate +5% and SP Recovery Rate +5%", 15, 4, 16, 4, 0]
	, [408, 6, "Green Ferus", 0, 3, 1, 15, 10, 0]
	, [409, 6, "Lady Tanee", "20% chance to obtain a [Banana] item when killing a monster.<BR>+100% HP restoration with it.", 15, -40, 16, 50, 0]
	, [410, 7, "Gremlin", "0.5% chance to obtain a [Bomber Steak] (STR+3) item when killing a [" + v_Race[2] + "] race monster.", 0]
	, [411, 7, "Beholder", 0, 220, 29, 0]
	, [412, 7, "Anopheles", "0.5% chance to obtain a [Tentacle Cheese Gratin] (AGI+3) item when killing an [" + v_Race[4] + "] race monster.", 0]
	, [413, 7, "Whikebine Tres", 0, 221, 80, 0]
	, [414, 7, "Errende Ebeece", 0, 221, 81, 0]
	, [415, 7, "Plasma", "0.5% chance to obtain any Elemental-proof Potion item when killing a monster.", 0]
	, [416, 7, "Baroness of Retribution", "0.5% chance to obtain a [Fried Sweet Potato] (LUK+3) item when killing an [" + v_Race[8] + "] race monster.", 0]
	, [417, 7, "Dragon Egg", "0.5% chance to obtain a [Honey Herbal Tea] (INT+3) item when killing a [" + v_Race[9] + "] race monster.", 0]
	, [418, 7, "Grove", "0.5% chance to obtain a [Fruit Mix] (DEX+3) item when killing a [" + v_Race[0] + "] race monster.", 0]
	, [419, 7, "Hydrolancer", 0, 221, 82, 0]
	, [420, 4, "RSX-0806", "Player cannot be pushed back.<BR>Armor becomes unbreakable.", 3, 3, 0]
	, [421, 4, "High Priest", 0, 221, 83, 0]
	, [422, 4, "Detardeurus", 0, 19, -20, 152, 100, 221, 84, 0]
	, [423, 6, "General Egnigem Cenia", "Recover 50 HP and 10 SP every 10 seconds.", 15, 10, 16, 10, 0]
	, [424, 2, "High Wizard", "Magical attacks ignore MDEF of [Non-boss] type targets.<BR>Consume 2000 SP when the headgear is unequipped.", 73, 100, 76, -100, 0]
	, [425, 2, "Vesper", "Ignores 30% MDEF of [Boss] type monsters.", 5, 2, 0]
	, [426, 2, "Margaretha Sorin", 0, 4, 1, 221, 85, 0]
	, [427, 1, "Dolor of Thanatos", 0, 178, 10, 0]
	, [428, 1, "Skeggiold", 0, 176, 2, 0]
	, [429, 100, 0, "[2 or more Injustice cards don't make you get more % of [Sonic Blows] being cast or give more ATK/LUK]", 17, 20, 6, 3, 0]
	, [430, 100, 0, "The chance to cast [Frost Diver] increases to 30%", 0]
	, [431, 100, 0, "30% chance to obtain a [Raw Fish] item when killing a [" + v_Race[5] + "] race monster.", 41, 30, 0]
	, [432, 100, 0, 0, 18, 3, 19, 3, 0]
	, [433, 100, 0, 0, 4, 4, 3, 4, 75, 30, 76, 30, 0]
	, [434, 100, 0, 0, 73, -10, 15, 20, 16, 20, 0]
	, [435, 100, 0, 0, 1, 3, 0]	//no mejorar
	, [436, 100, 0, 0, 18, 3, 3, 3, 0]
	, [437, 100, 0, 0, 86, 20, 0]
	, [438, 100, 0, "[Sleep] chance increases to 9%", 0]
	, [439, 100, 0, "[Blind] chance increases to 9%", 0]
	, [440, 100, 0, "[Curse] chance increases to 9%", 0]
	, [441, 100, 0, 0, 7, 1, 0]
	, [442, 100, 0, 0, 9, 18, 0]
	, [443, 999, 0]
	, [444, 999, 0]
	, [445, 999, 0]
	, [446, 999, 0]
	, [447, 999, 0]
	, [448, 100, 0, 0, 13, 300, 14, 60, 0]
	, [449, 100, 0, 0, 221, 86, 0]
	, [450, 100, 0, "5% chance to obtain a [Red Potion] item when killing a monster.<BR><b>[Swordman Class]</b><br>+50% HP restoration with [Red Potion], [Yellow potion], and [White Potion] items.", 1, 10, 15, 20, 75, 50, 221, 87, 0]
	, [451, 100, 0, "Movement Speed +5%<BR>Disables the auto-casting of [" + skillName(167) + "].<BR>Recover 1 SP when dealing ATK based damage.<BR><b>[Thief Class]</b><br>Removes the consumption of gemstones from skills that usually would.", 1, 5, 2, 5, 12, 5, 0]
	, [452, 100, 0, "<b>[Acolyte Class]</b><br>-30% damage from [" + v_Race[6] + "] and [" + v_Race[1] + "] race monsters.<BR>+5% experience from [" + v_Race[6] + "] and [" + v_Race[1] + "] race monsters.", 3, 10, 73, -10, 370, -10, 0]
	, [453, 100, 0, "<b>[Archer Class]</b><br>1% chance to cause [Coma] status effect on [" + v_Race[2] + "] race monsters.<BR>+5% experience from [" + v_Race[2] + "] race monsters.", 25, 20, 2, 5, 5, 3, 86, 20, 0]
	, [454, 100, 0, "<b>[Mage Class]</b><br>MATK +3%, Casting Time -15%", 13, 500, 18, 5, 19, 5, 5051, 10, 5054, 10, 5056, 10, 0]
	, [455, 100, 0, "Recover 2 SP when dealing ATK based damage.<BR><b>[Merchant Class]</b><br>0.05% chance to obtain an [Old Purple Box] when killing a monster.<BR>20% chance to reflect damage received from a targeted magical attack.", 6, 10, 5065, 20, 0]
	, [456, 100, 0, "<b>[Crusader Class]</b><br>Armor becomes [" + v_Element[6] + "] element.", 1, 1, 4, 1, 18, 2, 76, 10, 73, -10, 5324, 10, 5284, 10, 0]
	, [457, 100, 0, "If the user knows [" + skillName(174) + "] Lvl 5, 5% chance to cast [" + skillName(174) + "] Lvl 5 when performing an ATK based attack.<BR><b>[Rogue Class]</b><br>All skill's SP cost -20%, Disables the auto-casting of [" + skillName(167) + "].", 1, 6, 2, 4, 5169, 10, 0]
	, [458, 100, 0, "<b>[Monk Class]</b><br>All skill's SP cost -10%, Skill casting cannot be interrupted (except in WoE)", 1, 3, 17, 25, 14, 80, 5197, 10, 5321, 10, 221, 88, 0]
	, [459, 100, 0, "<b>[Bard/Dancer Class]</b><br>When receiving ATK based damage, 5% chance to cast [Tarot Card of Fate] Lvl 5 on your attacker.", 5, 5, 5199, 10, 5207, 10, 5292, 5, 0]
	, [460, 100, 0, "<b>[Sage Class]</b><br>Casting Time -20%<BR>20% chance to reflect damage received from a targeted magical attack.<br>1% chance to obtain a [Red Gemstone] or [Yellow Gemstone] items when killing a monster.", 4, 3, 5057, 10, 5133, 10, 0]
	, [461, 100, 0, "Recover 1 SP when dealing ATK based damage.<BR><b>[Alchemist Class]</b><br>1% chance to cast [Adrenaline Rush] on each ATK based attack.<BR>0.1% chance to obtain a [Stem] item when killing a monster.<BR>0.03% chance to obtain a [Coating Bottle] item when killing a monster.", 1, 4, 15, 7, 16, 7, 5065, 20, 0]
	, [462, 1, "Drosera", "Adds CRIT +15 for long range ATK based attacks", 0]
	, [463, 1, "Atroce", "When performing an ATK based attack, 0.5% chance you gain ASPD +100% for 10 seconds.", 17, 25, 0]
	, [464, 1, "Sword Guardian", "When equipped with a [Sword] or [Two-Handed Sword] class weapon: CRIT +5, HIT +5, and [Bowling Bash] skill damage +25%", 0]
	, [465, 1, "Bow Guardian", "When equipped with a [Bow] class weapon: CRIT +5, HIT +5, and [Arrow Shower] skill damage +50%", 0]
	, [466, 1, "Necromancer", "When equipped with a [One-handed Rod] class weapon: INT +1 and 2% MDEF bypassing.", 0]
	, [467, 3, "Hodremlin", "When receiving ATK based damage, 0.3% chance you gain Perfect Dodge +30 for 10 seconds.", 190, 15, 191, 15, 192, 15, 0]
	, [468, 3, "Seeker", 0, 220, 30, 159, 30, 19, 10, 0]
	, [469, 3, "Muscipular", 0, 221, 89, 221, 90, 0]
	, [470, 3, "Flame Skull", "When receiving ATK based damage, 5% chance to cause [Stun] status effect on your attacker.<BR>When receiving ATK based damage, 5% chance to cause [Curse] status effect on your attacker.<BR>When receiving ATK based damage, 5% chance to cause [Blind] status effect on your attacker.<BR>When receiving ATK based damage, 5% chance to cause [Stone Curse] status effect on your attacker.", 151, 30, 153, 30, 154, 30, 159, 30, 0]
	, [471, 2, "Vanberk", "When dealing ATK based damage, 0.5% chance you gain CRIT +100 for 5 seconds.", 1, 2, 0]
	, [472, 2, "Isilla", "When dealing MATK based damage, 5% chance you gain FLEE +30 and Casting time -50% for 5 seconds.", 4, 2, 0]
	, [473, 2, "Knocker", "0.1% chance to obtain a [Rough Elunium] or [Rough Oridecon] items when killing a monster.", 30, 5, 0]
	, [474, 2, "Banshee", "If equipped by a [Mage] class job:<br>MaxSP +100, MaxHP -100<br>[Soul Strike], [Napalm Beat], and [Napalm Vulcan] skills damage +20%", 0]
	, [475, 4, "Gloom Under Night", 0, 46, 40, 47, 40, 38, 40, 36, 40, 0]
	, [476, 4, "Agav", "<b>[Mage Class]</b><br>SP + 100", 89, 5, 18, -10, 0]
	, [477, 4, "Echio", "If equipped by a [Swordman] class job: MaxHP +500", 17, 15, 0]
	, [478, 4, "Ktullanux", 0, 43, 50, 221, 91, 0]
	, [479, 4, "Byorgue", "<b>[Rogue Class]</b><br>ATK based damage +10% [versus Normal/Boss/Guardian]<br>MATK +10%", 0]
	, [480, 5, "Roween", 0, 9, 5, 11, 3, 41, 10, 115, 15, 0]
	, [481, 5, "Salamander", 0, 5122, 40, 5125, 40, 0]
	, [482, 5, "Kasa", 0, 221, 92, 221, 93, 0]
	, [483, 5, "Magmaring", 0, 17, 5, 42, 10, 112, 15, 113, 15, 0]
	, [484, 6, "Ice Titan", "When receiving ATK or MATK based damage, 0.3% chance you gain DEF +10 for 10 seconds.", 3, 2, 0]
	, [485, 6, "Zombie Slaughter", "Recover 50 HP when killing a monster with ATK based damage.", 37, 1, 177, 1, 0]
	, [486, 6, "Fallen Bishop", 0, 89, 10, 16, -50, 177, 50, 178, 50, 0]
	, [487, 7, "Snowier", "20% chance to obtain an [Ice Cream] item when killing a monster.<BR>[Ice Cream] item recover +100%", 0]
	, [488, 7, "Siroma", 0, 5054, 25, 7054, -25, 0]
	, [489, 7, "Gazeti", 0, 221, 94, 0]
	, [490, 7, "Galion", 0, 8, 5, 41, 5, 0]
	, [491, 7, "Stapo", 0, 220, 31, 220, 32, 0]
	, [492, 7, "Ifrit", "Increases your ATK, HIT, and CRIT by 1 for every 10 job levels you have.<BR>When receiving ATK based damage, 0.1% chance to cast [Earthquake] Lvl 2", 0]
	, [493, 7, "Imp", 0, 5051, 25, 7051, -25, 0]
	, [494, 7, "Ragged Zombie", "When dealing ATK based damage, 0.1% chance to cause [Bleeding] status effect on your target.", 117, 5, 37, 1, 177, 1, 0]
	, [495, 7, "Hellhound", "[Meat] item recover +100%<BR>When dealing ATK based damage, 0.5% chance to cause [Bleeding] status effect on your target.", 8, 1, 0]
	, [496, 100, 0, "Recover 2 SP when killing a [" + v_Race[7] + "] race enemy with ATK based damage.", 0]
	, [497, 7, "Beelzebub NPC Card", "<br><b>* WARNING:Having more then one Beelzebub NPC Card doesn't give -60% cast, it will give you -30% always.</b>", 73, -30, 0]
	, [498, 1, "Hillslion", 0, 17, 25, 0]
	, [499, 1, "Centipede Larva", 0, 4, 1, 88, 3, 0]
	, [500, 4, "Banshee Master", 0, 4, 1, 88, 10, 0]
	, [501, 4, "Entweihen Crothen", 0, 88, 100, 0]
	, [502, 3, "Aqua Elemental", 0, 331, 20, 41, 5, 0]
	, [503, 3, "Centipede", 0, 335, 20, 45, 5, 0]
	, [504, 3, "Cornus", 0, 336, 20, 46, 5, 0]
	, [505, 3, "Draco (old)", 0, 333, 20, 43, 5, 0]
	, [506, 3, "Dark Shadow", 0, 337, 20, 47, 5, 0]
	, [507, 3, "Luciola Vespa", 0, 334, 20, 44, 5, 0]
	, [508, 3, "Tatacho (old)", 0, 330, 20, 40, 5, 0]
	, [509, 2, "Rata", "When dealing MATK based damage, 3% chance you gain After-cast Delay -20% for 5 seconds.", 4, 2, 0]
	, [510, 2, "Dark Pinguicula", "When killing a monster, 0.2% chance to obtain an Aloe.", 17, 10, 0]
	, [511, 2, "Duneyrr", "When dealing ATK based damage, 1% chance you gain Perfect Dodge +10 for 4 seconds.", 17, 10, 0]
	, [512, 2, "Phylla", "When dealing ATK based damage, 1.5% chance you gain CRIT +20 for 4 seconds.", 2, 1, 5, 1, 0]
	, [513, 3, "Rhyncho", "<b>[Priest Classes]</b><BR>INT +1<br><hr><b>When compounded into a [Rose of Eden]:</b><br>Your healing effectiveness +5%", 190, 12, 191, 12, 192, 12, 99, 5, 0]		//COMPROBAR SCRIPT
	, [514, 1, "Zakudam", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:orange'>Demi-Human</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:orange'>Demi-Human</b>] race monsters.", 0]
	, [515, 1, "Pinguicula", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:green'>Insect</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:green'>Insect</b>] race monsters.", 0]
	, [516, 1, "Nepenthes", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:#00DD00'>Plant</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:#00DD00'>Plant</b>] race monsters.", 0]
	, [517, 1, "Naga", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:blue'>Fish</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:blue'>Fish</b>] race monsters.", 0]
	, [518, 1, "Hell Apocalypse", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:black'>Demon</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:black'>Demon</b>] race monsters.", 0]
	, [519, 1, "Draco Egg", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:red'>Dragon</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:red'>Dragon</b>] race monsters.", 0]
	, [520, 1, "Cobalt Mineral", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:#9F9E9B'>Formless</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:#9F9E9B'>Formless</b>] race monsters.", 0]
	, [521, 1, "Bradium Golem", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:brown'>Brute</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:brown'>Brute</b>] race monsters.", 0]
	, [522, 1, "Ancient Tree", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:purple'>Undead</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:purple'>Undead</b>] race monsters.", 0]
	, [523, 6, "Heavy Metaling", "If equipped by a [Merchant] class job: [Cart Revolution] skill damage +50% and [Cart Revolution] SP cost +12", 1, 2, 0]
	, [524, 1, "Aunoe", 0, 70, 20, 0]
	, [525, 1, "Fanat", "<b>If compounded on a two-handed sword:</b><BR>ASPD +5%", 17, 10, 0]
	, [526, 1, "Tendrillion (aRO)", "<b>[Refine Rate 12~20]</b> ATK +35<br><b>[Refine Rate 14~20]</b> CRIT +10<br><b>* WARNING: Refines above +10 are not implemented at Pre-Renewal.</b>", 10, 5, 0]
	, [527, 4, "Nidhoggur's Shadow", 0, 4, 5, 150, 30, 151, 30, 152, 30, 153, 30, 154, 30, 155, 30, 156, 30, 157, 30, 158, 30, 159, 30, 0]
	, [528, 1, "Beholder Master", "<b>If compounded on a ranged weapon or Huuma:</b><BR>ASPD +5%", 25, 3, 0]
	, [529, 4, "Naght Seiger", "", 5046, 30, 5047, 30, 5277, 30, 0]
	, [530, 4, "Hardrock Mammoth", "<b>[Refine Rate 7~10]</b> HP +10%<br><b>[Refine Rate 9~10]</b> HP+3%", 18, 5, 0]
	, [531, 4, "Hardrock Mammoth (aRO)", "<b>[Refine Rate 7~10]</b> MaxHP +3%<br><b>[Refine Rate 9~20]</b> DEF +10, MaxHP +10%", 18, 3, 0]
	, [532, 1, "Tendrillion", "<b>[Refine Rate 7~10]</b> CRIT +5<br><b>[Refine Rate 9~10]</b> Critical Damage +10%", 10, 5, 0]
	, [533, 4, "Nidhoggur Shadow (aRO)", "If equipped by a [High Wizard] or [Warlock] jobs: Casting Time -50%", 4, 5, 0]
	, [534, 1, "(unused)", 0, 0]
	, [535, 1, "(unused)", 0, 0]
	, [536, 1, "(unused)", 0, 0]
	, [537, 3, "Alnoldi", 0, 53, 30, 0]
	, [538, 3, "Draco", 0, 332, 20, 42, 5, 0]
	, [539, 3, "Tatacho", 0, 330, 20, 40, 5, 0]
	, [540, 2, "Dolomedes", "If equipped by an [Archer] class job: DEX +(refine/3)", 5, 2, 0]
	, [541, 7, "Scaraba", "", 16, -1, 88, 20, 0]
	, [542, 7, "Gold Scaraba", "", 17, 20, 15, -1, 0]
	, [543, 2, "Gold Queen Scaraba", "<b>[Refine Rate 9~20]</b> Adds an additional 5% resistance to [" + v_Race[4] + "] race monsters.", 4, 3, 54, 10, 0]
	, [544, 1, "Beholder Master (aRO)", "<b>If compounded on a bow:</b><br><b>[Refine Rate 7~10]</b> ASPD +1<br><b>[Refine Rate 9~10]</b> Additional ASPD +1", 25, 3, 0]
	, [545, 2, "Duneyrr (old)", "When dealing ATK based damage, 1% chance you gain ASPD +5% for 10 seconds.", 17, 10, 0]
	, [546, 7, "Kafra Blossom", "2% chance to increase resistance to [" + v_Element[3] + "], [" + v_Element[4] + "], [" + v_Element[1] + "], [" + v_Element[2] + "], [" + v_Element[6] + "], [" + v_Element[7] + "] elements by 10% for 10 seconds when receiving magic damage.", 61, 10, 62, 10, 63, 10, 64, 10, 66, 10, 67, 10, 0]
	, [547, 1, "Pom Spider", "", 49, 20, 0]
	, [548, 1, "Little Fatum", "When dealing MATK based damage, 5% chance to cause [Silence] status effect on your target.", 0]
	, [549, 1, "Miming", "When dealing melee ATK based damage, 0'3% chance to cause [Deep Sleep] status effect on your target.", 0]
	, [550, 2, "Angra Mantis", "If equipped by a [Thief] class job: Critical Damage +(refine/2)", 70, 2, 0]
	, [551, 2, "Parus", "If equipped by a [Acolyte] class job: Your [Heal], [Sanctuary], and [" + skillName(245) + "] effectiveness +(refine/2)%", 99, 3, 0]
	, [552, 4, "Comodo", "<b>* WARNING: Renewal Card.</b>", 18, 50, 9, -25, 0]
	, [553, 5, "Leak", "50% chance to cause Confusion on target enemy when hit by magic damage.<BR><b>* WARNING: Renewal Card.</b>", 1, 3, 0]
	, [554, 5, "Randel (aRO)", "Enables the use of [" + skillName(157) + "] Lvl 1", 9, 10, 0]
	, [555, 4, "Paladin Randel (aRO)", "<b>[Base INT >= 110]</b> MaxHP +10%", 221, 164, 15, 10, 0]
	, [556, 5, "Flamel (aRO)", "Increase HP restored with Red, Orange, Yellow and White Potion by 200%", 9, 10, 0]
	, [557, 4, "Biochemist Flamel (aRO)", "<b>[Base STR >= 110]</b> CRI +20", 221, 165, 10, 20, 0]
	, [558, 5, "Celia (aRO)", "Enables the use of [" + skillName(240) + "] Lvl 1", 9, 10, 0]
	, [559, 4, "Scholar Celia (aRO)", "<b>[Base DEX >= 110]</b> MATK +7%", 221, 166, 89, 7, 0]
	, [560, 5, "Chen (aRO)", "Enables the use of [" + skillName(185) + "] Lvl 2", 9, 10, 0]
	, [561, 4, "Champion Chen (aRO)", "<b>[Base AGI >= 110]</b> ATK +7%", 221, 167, 87, 7, 0]
	, [562, 5, "Gertie (aRO)", "Enables the use of [Close Confine] Lvl 1", 9, 10, 0]
	, [563, 4, "Stalker Gertie (aRO)", "<b>[Base LUK >= 110]</b> HIT +20", 221, 168, 8, 20, 0]
	, [564, 5, "Alphoccio (aRO)", "<b>[Bard Class]</b> MaxHP +10%, MaxSP +5%.", 9, 10, 0]
	, [565, 4, "Minstel Alphoccio (aRO)", "<b>[Base VIT >= 110]</b> FLEE +20<BR>Enables use of [" + skillName(204) + "] Lvl 10 (this skill can only be used when an instrument is equipped).", 9, 20, 0]
	, [566, 5, "Trentini (aRO)", "<b>[Dancer Class]</b> MaxHP +10%, MaxSP +5%.", 9, 10, 0]
	, [567, 4, "Gypsi Trentini (aRO)", "<b>[Base VIT >= 110]</b> FLEE +20<BR>Enables use of [" + skillName(212) + "] Lvl 10 (this skill can only be used when a whip or an instrument is equipped).", 9, 20, 0]
	// custom alfheim cards
	, [568, 6, "Death Witch", 0, 16, -50, 176, 50, 170, 50, 0]
	, [569, 4, "Lude Gal", 0, 15, -8, 16, 5, 0]
	, [570, 100, 0, 0, 16, 5, 0]
	, [571, 4, "Ghost Cube", 0, 15, 8, 16, -5, 0]
	, [572, 100, 0, 0, 15, 7, 0]
	, [573, 5, "Gan Ceann", 0, 17, 5, 47, 10, 0]
	, [574, 1, "Brutal Murderer", 0, 31, 20, 0]
	, [575, 2, "Blue Moon Loli Ruri", "", 4, 2, 99, 10, 370, 10, 0]
	, [576, 1, "Pierrotzoist", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:#BFBEBB'>Ghost</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:#BFBEBB'>Ghost</b>] element monsters.", 0]
	, [577, 1, "Grote", 0, 49, 20, 0]
	, [578, 2, "Disguiser", "<b>[Refine Rate 9~10]</b> Deal +5% ranged ATK based damage.", 5, 1, 0]
	, [579, 1, "Sropho", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:blue'>Water</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:blue'>Water</b>] element monsters.", 0]
	, [580, 1, "Sedora", 0, 17, 10, 70, 15, 0]
	, [581, 5, "Pot Dofle", 0, 60, 15, 61, 10, 0]
	, [582, 2, "King Dramoh", "<b>For every 3 refine levels:</b> STR+1", 0]
	, [583, 4, "Kraken", "Enables the use of [Deluge] Lvl 1", 381, 10, 0]
	, [584, 3, "Scout Basilisk", 0, 190, 20, 191, 15, 192, 10, 0]
	, [585, 3, "Combat Basilisk", 0, 191, 20, 192, -15, 0]
	, [586, 3, "Charge Basilisk", 0, 192, 20, 191, -15, 0]
	, [587, 7, "Trans Spore", 0, 4, 1, 3, 1, 0]
	, [588, 1, "Fruit Pom Spider", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:red'>Fire</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:red'>Fire</b>] element monsters.", 0]
	, [589, 1, "Jungle Mandragora", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:#00CC00'>Wind</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:#00CC00'>Wind</b>] element monsters.", 0]
	, [590, 1, "Big Eggring", 0, 221, 173, 0]
	, [591, 7, "Eggring", 0, 5056, 25, 7056, -25, 0]
	, [592, 2, "Leaf Lunatic", 0, 13, 150, 0]
	, [593, 1, "Sweet Drops", 0, 88, 10, 0]
	, [594, 1, "Hunter Wolf", 0, 8, 10, 0]
	, [595, 6, "Baba Yaga", "<b>For each refine level:</b> FLEE+2", 0]
	, [596, 3, "Les", 0, 334, 25, 0]
	, [597, 7, "Mavka", "<b>[Ninja Class]</b><BR>[<b style='color:red'>Fire</b>] property magic damage +7%<BR>[<b style='color:brown'>Earth</b>] property magic damage +7%", 4, 1, 5, 1, 0]
	, [598, 2, "Uzhas", "<b>[Refine Rate 9~10]</b><br>Deal +5% MATK based damage against [<b style='color:brown'>Earth</b>] element monsters.", 342, 5, 0]
	, [599, 4, "Wood Goblin", "", 41, 7, 42, 7, 331, -7, 332, -7, 0]
	, [600, 2, "Gopinich", "Recover 5 SP when dealing ATK based damage.", 370, 50, 0]
	, [601, 4, "Boitata", 0, 44, 40, 42, 40, 34, 40, 32, 40, 0]
	, [602, 2, "Curupira", "<b>[Refine Rate 9~10]</b><br>Deal +5% MATK based damage against [<b style='color:blue'>Water</b>] element monsters.", 341, 5, 0]
	, [603, 2, "Headless Mule", "<b>[Refine Rate 9~10]</b><br>Deal +5% MATK based damage against [<b style='color:red'>Fire</b>] element monsters.", 343, 5, 0]
	, [604, 6, "Iara", "[Status Recovery] After-cast Delay -50%", 16, 10, 0]
	, [605, 2, "Jaguar", "<b>[Refine Rate 9~10]</b><br>Deal +5% MATK based damage against [<b style='color:#00CC00'>Wind</b>] element monsters.", 344, 5, 0]
	, [606, 6, "Piranha", "<b>[Refine Rate 9~10]</b><br>Deal +5% MATK based damage against [<b style='color:blue'>Fish</b>] race monsters.", 175, 5, 0]
	, [607, 2, "Toucan", "<b>[Refine Rate 9~10]</b><br>Deal +5% MATK based damage against [<b style='color:purple'>Poison</b>] element monsters.", 345, 5, 0]
	, [608, 2, "* All Race Magical +8%", "", 170, 8, 171, 8, 172, 8, 173, 8, 174, 8, 175, 8, 176, 8, 177, 8, 178, 8, 179, 8, 0]
	, [609, 2, "* All Element Magical +8%", "", 340, 8, 341, 8, 342, 8, 343, 8, 344, 8, 345, 8, 346, 8, 347, 8, 348, 8, 349, 8, 0]
	// patch a1
	, [610, 7, "Contaminated Raydric", "", 45, 5, 0]
	, [611, 5, "Contaminated Raydric Archer", "", 61, 20, 62, 20, 63, 20, 64, 20, 65, 20, 66, 20, 67, 20, 68, 20, 69, 20, 60, -10, 0]
	, [612, 1, "Corrupted Wanderer", "<hr><b>When equipping [Corrupted Wanderer Card]+[Sigrun's Wings] at the same time:</b><br>When dealing ATK based damage, 2% chance you gain 1% of the damage inflicted on your target as SP.", 28, 5, 29, 5, 0]
	, [613, 1, "Cursed Raydric", "", 88, 15, 0]
	, [614, 6, "Cursed Raydric Archer", "", 5, 1, 15, 10, 0]
	, [615, 5, "Flame Ghost", "", 60, 15, 63, 10, 0]
	, [616, 5, "Ice Ghost", "", 17, 5, 43, 10, 0]
	, [617, 1, "Gigantes", "<b>[Weapon Level 1~2]</b> On all monsters ignore 1% DEF<BR><b>[Weapon Level 3]</b> On all monsters ignore 2% DEF<br><b>[Weapon Level 4]</b> On all monsters ignore 3% DEF<br><hr><b>When equipped with a [Axe] or [Two-Handed Axe] class weapon:</b><br>DEF Pierce values on the card are doubled.", 17, 20, 0]
	, [618, 3, "Khalitzburg Knight", "", 190, -15, 191, 18, 192, 18, 0]
	, [619, 1, "White Knight", "", 17, 10, 28, 13, 29, 13, 0]
	, [620, 100, 0, 0, 357, 4, 358, 4, 0]
	, [621, 100, 0, 0, 28, 12, 29, 12, 191, 2, 192, 2, 0]
	, [622, 6, "Polluted Spider Queen", "Auto Cast Rate +100%", 12, 5, 0]
	, [623, 2, "Polluted Dark Lord", "", 1, 2, 292, 30, 0]
	, [624, 5, "Bloody Wraith", "", 15, 5, 0]
	, [625, 6, "Corrupted Bishop", "", 4, 1, 15, 10, 0]
	, [626, 1, "Corrupted Duke", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:#A89682'>Neutral</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:#A89682'>Neutral</b>] element monsters.", 0]
	, [627, 6, "Dark Chimera", "", 15, 6, 11, 4, 0]
	, [628, 7, "Mutating Prisoner", 0, 5132, 25, 7132, -25, 0]
	, [629, 7, "Royal Clergy", "<b>[Ninja Class]</b><BR>[<b style='color:blue'>Water</b>] property magic damage +7%<BR>[<b style='color:#00CC00'>Wind</b>] property magic damage +7%", 4, 1, 5, 1, 0]
	, [630, 2, "Royal Druid", "<b>[Refine Rate 9~10]</b> On all monsters, ignore 5% MDEF", 4, 1, 0]
	, [631, 3, "Standing Soul", "", 337, 25, 0]
	, [632, 3, "Undead Noble", "", 5, 1, 74, 5, 0]
	, [633, 4, "Himmelmez", "", 48, 40, 49, 40, 37, 40, 31, 40, 0]
	, [634, 7, "Contaminated Sting", "", 2, 3, 0]
	, [635, 5, "Frozen Gargoyle", "", 60, 10, 5199, 10, 5207, 10, 0]
	, [636, 1, "Roaming Spellbook", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:brown'>Earth</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:brown'>Earth</b>] element monsters.", 0]
	, [637, 1, "Corruption Root", "When performing an ATK based attack, 5% chance to cast [Wide Curse], [Wide Sleep], and [Wide Stone]", 17, 20, 0]
	, [638, 6, "King Schmidt", "When performing an ATK based attack, 7.5% chance to cast [Sonic Blow] Lvl 5", 17, 25, 0]
	, [639, 7, "Moskillro", "", 5, 1, 17, 5, 123, 5, 0]
	, [640, 7, "Brain Sucker", "", 1, 1, 17, 5, 123, 5, 0]
	, [641, 7, "Grave Worm", "", 4, 1, 295, 2, 123, 5, 0]
	, [642, 3, "Unidentified Creature", "", 11, 30, 9, -100, 0]
	, [643, 1, "Warden Kades", "<b>[Weapon Level 3~4]</b><br><b style='color:#A89682'>Neutral</b> property magic damage +6%<BR><b style='color:#CDCD00'>Holy</b> property magic damage +6%<BR><b style='color:#000000'>Shadow</b> property magic damage +6%", 88, 25, 0]
	, [644, 6, "Dark Priest", "<b>[Rogue Class]</b> Auto Cast Rate +20%", 2, 1, 0]
	, [645, 1, "Zombie Guard", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:purple'>Undead</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:purple'>Undead</b>] element monsters.", 0]
	// patch a2.0
	, [646, 1, "Big Ben", "<b>[Weapon Level 1~2]</b> Deal +2% MATK based damage against [<b style='color:#9F9E9B'>Formless</b>] and [<b style='color:#000000'>Demon</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +5% MATK based damage against [<b style='color:#9F9E9B'>Formless</b>] and [<b style='color:#000000'>Demon</b>] race monsters.", 0]
	, [647, 1, "Big Bell", "", 30, 10, 36, 10, 0]
	, [648, 3, "Neo Punk", "", 50, 20, 56, 20, 0]
	, [649, 100, "Twisted Time Set", "", 310, 10, 316, 10, 0]
	, [650, 100, "Nightmare Clock Tower Set", "", 300, 10, 306, 10, 0]
	, [651, 6, "Time Keeper", "<b>[Refine Rate 9~10]</b> MaxHP +10% and MaxSP +10%", 3, 2, 0]
	, [652, 5, "Arc Elder", "<b>For each refine level:</b> [<b style='color:brown'>Earth</b>] property magic damage +1%", 60, 15, 0]
	, [653, 7, "Owl Viscount", "<b>Only Left Accessory</b>", 12, 3, 0]
	, [654, 4, "Owl Marquees", "User is no longer detected by monsters who can normally see through invisibility.", 0]
	, [655, 5, "Decorated Evil Tree", "", 17, 5, 46, 10, 0]
	, [656, 7, "Vicious Cookie", "", 46, 5, 0]
	, [657, 1, "Corrupt Cruiser", "", 25, 8, 221, 175, 0]
	, [658, 3, "Evil Dwelling Box", "", 339, 20, 0]
	, [659, 1, "Creepy Demon", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:#CDCD40'>Angel</b>] race monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:#CDCD40'>Angel</b>] race monsters.", 0]
	, [660, 5, "Malicious Baby Ghost", "", 60, 15, 68, 10, 0]
	, [661, 5, "Dancing Marionette Ghost", "", 60, 15, 67, 10, 0]
	, [662, 6, "Abandoned Teddy Bear", "<b>For each refine level:</b> MaxSP +1%<br><b>[Refine Rate 9~10]</b> DEX -3", 16, 8, 0]
	, [663, 1, "Celine Kimi", "When performing a MATK based attack, 0.5% chance to cast [Wide Critical Wounds].<br>This rate is increased by an additional 1% for every 3 upgrade levels of the compounded item.", 89, 10, 0]
	// patch a3.0
	, [664, 3, "Rigid Blazer", "", 51, 20, 59, 20, 0]
	, [665, 6, "Rigid Nightmare Terror", "", 3, 2, 15, 5, 0]
	, [666, 2, "Rigid Sky Deleter", "", 3, 1, 13, 100, 0]
	, [667, 6, "Rigid Earth Deleter", "", 2, 1, 10, 2, 70, 1, 0]
	, [668, 1, "Rigid Explosion", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:#000000'>Shadow</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:#000000'>Shadow</b>] element monsters.", 0]
	, [669, 7, "Rigid Kaho", "", 43, 5, 0]
	, [670, 3, "Rigid Lava Golem", "", 333, 25, 0]
	// patch a3.7
	, [671, 3, "Toxious", "", 332, 25, 0]
	, [672, 1, "Neo Mineral", "", 40, 20, 0]
	, [673, 4, "Jewel Ungoliant", "", 30, 40, 33, 40, 0]
	// patch a4
	, [674, 5, "Coyote", "", 60, 15, 62, 10, 0]
	, [675, 1, "Tumblering", "<b>[Weapon Level 1~2]</b> Deal +3% MATK based damage against [<b style='color:#bb24bb'>Poison</b>] element monsters.<BR><b>[Weapon Level 3~4]</b> Deal +8% MATK based damage against [<b style='color:#bb24bb'>Poison</b>] element monsters.", 0]
	, [676, 2, "Shotgun Buffalo Bandit", "<b>For every 2 refine levels:</b> LUK +1", 2, 2, 0]
	, [677, 7, "Revolver Buffalo Bandit", "", 5, 2, 17, 8, 0]
	, [678, 7, "Scimitar Buffalo Bandit", "", 1, 2, 17, 8, 0]
	, [679, 5, "Elite Shotgun Buffalo Bandit", "", 4, 2, 172, 5, 0]
	, [680, 4, "Elite Revolver Buffalo Bandit", "<b>[Base LUK >= 40]</b><br>Critical Damage +5%", 17, 12, 0]
	, [681, 4, "Elite Scimitar Buffalo Bandit", "<b>[Base STR >= 70]</b><br>On all monsters ignore 10% DEF", 17, 12, 0]
	, [682, 5, "Spider Chariot", "", 221, 180, 0]
	, [683, 3, "Muspellskoll", "", 78, 30, 63, 30, 0]
	, [684, 4, "Kiel Hyre", "", 13, 200, 14, 75, 0]
    , [685, 7, "Diligent Andre", "", 4, 1, 74, 2, 0]
    , [686, 7, "Diligent Deniro", "", 17, 5, 8, 10, 0]
    , [687, 1, "Diligent Piere", "", 12, 2, 221, 181, 0]
    , [688, 4, "Silent Maya", "", 172, 25, 173, 25, 370, 50, 0]
];

// positions (1 = weapon, 2 = headgear, 3 = shield, 4 = armor, 5 = garment, 6 = footgear, 7 = accessory, 100 = combos)
// races (def = 5 + num, atk = 3 + num) (0 = formless, 1 = undead, 2 = brute, 3 = plant, 4 = insect, 5 = fish, 6 = demon, 7 = demi-human, 8 = angel, 9 = dragon)
// elements (atk = 4 + num, def1 = 6, def2 = 33 + num) (0 = neutral, 1 = water, 2 = earth, 3 = fire, 4 = wind, 5 = poison, 6 = holy, 7 = shadow, 8 = ghost, 9 = undead)
// sizes (27 = small, 28 = medium, 29 = large)

CardNum = m_Card.length - 1;

m_CardSort = [	//cards renewal comentadas
	[0, 1, 2, 3, 608, 609, 156, 31, 25, 522, 11, /* 112, */ 107, 223, 356, 463, 524, 305, 45, 355, 528, /* 544, */ 647, 646, 590, 361, 217, 465, 521, 163, 574, 15, 265, 160, 663, 499, 325, 520, 657, 626, 612, 637, 259, 659, 279, 613, /*311,*/ 169, 28, 20, 329, 687, 427, 42, 519, 47, 21, 32, 343, 5, 462, 215, 19, 244, 158, 4, 359, 525, 110, 8, 17, 588, 253, 263, 617, 14, 286, 288, 287, 39, 277, 577, 518, 167, 498, 6, 159, 46, 13, 255, 235, 589, 26, 289, 290, 164, 168, 382,/*548,*/219, 7, 165, 37, 23, 398, 35, 161, 166, 157, 36,/*549,*/30, 328, 40, 380, 517, 466, 672, 516, 345, 252, 27, 353, 12, 360, 44, 576, 515, 171, 33,/*547,*/170, 111, 668, 636, 390, 22, 109, 16, /* 108, */ 580, 254, 43, 428, 29, 9, 34, 162, 41, 579, 366, 313, 18, 593, 464, /* 526, */ 532, 233, 296, 675, 323, 24, 363, 643, 619, 10, 514, 38, 326, 645, 319, "NULL"],
	//[0, 1, 2, 3, 608, 609, 156, 31, 25, 522, 11, /* 112, */ 107, 223, 356, 463, 524, 305, 45, 355, 528, /* 544, */ 590, 361, 217, 465, 521, 163, 574, 15, 265, 160, 499, 325, 520, 626, 612, 637, 259, 279, 613, 311, 169, 28, 20, 329, 427, 42, 519, 47, 21, 32, 343, 5, 462, 215, 19, 244, 158, 4, 359, 525, 110, 8, 17, 588, 253, 263, 617, 14, 286, 288, 287, 39, 277, 577, 518, 167, 498, 6, 159, 46, 13, 255, 235, 589, 26, 289, 290, 164, 168, 382,/*548,*/219, 7, 165, 37, 23, 398, 35, 161, 166, 157, 36,/*549,*/30, 328, 40, 380, 517, 466, 516, 345, 252, 27, 353, 12, 360, 44, 576, 515, 171, 33,/*547,*/170, 111, 636, 390, 22, 109, 16, /* 108, */ 580, 254, 43, 428, 29, 9, 34, 162, 41, 579, 366, 313, 18, 593, 464, /* 526, */ 532, 233, 296, 323, 24, 363, 643, 619, 10, 514, 38, 326, 645, 319, "NULL"],
	[0, 153,/*550,*/221, 332, 474, 179, 575, 298, 339, 114, 273, 602, 320, 510, 51, 578, 342, /* 540, */ 511, /* 545, */ 49, 337, 284, 176, 116, 303, 213, 117, /* 543, */ 600, 274, 603, 424, 175, 234, 472, 605, 177, 357, 582, 473, 397, 592, 387, 178, 118, 426, 115, 374, 119, 376, 50, 52,/*551,*/347, 120, 512, 623, 509, 383, 666, 630, 180, 676, 113, 309, 607, 174, 632, 598, 471, 425, 48, 239, 238, "NULL"],
	[0, 153,/*550,*/221, 332, 474, 179, 575, 298, 339, 114, 273, 602, 320, 510, 51, 578, 342, /* 540, */ 511, /* 545, */ 49, 337, 284, 176, 116, 303, 213, 117, /* 543, */ 600, 274, 603, 424, 175, 234, 472, 605, 177, 357, 582, 473, 397, 592, 387, 178, 118, 426, 115, 374, 119, 376, 50, 52,/*551,*/347, 120, 512, 623, 509, 383, 666, 630, 180, 676, 113, 309, 607, 174, 598, 471, 425, 48, 239, 238, "NULL"],
	[0, 154, 155, 231, /* 537, */ 55, 245, 54, 122, 502, 222, 124, 58, 503, 586, 585, 504, 506, 173, /* 505, */ 538, 307, 658, 241, 470, 126, 348, 467, 62, 61, 618, 596, 507, 127, 63, 123, 272, 125, 469, 683, 375, 648, 57, 358, 368, 59, 513, 664, 670, 584, 468, 60, 631, 121, 310, 396, /*508,*/ 539, 172, 56, 53, 249, 671, 642, 306, "NULL"],
	[0, 476, 194, 282, 191, 136, 224, 232, 225, 190, 135, 69, 281, 500, 137,/*557,*/364, 601, 479,/*561,*/264, 280,/*552,*/128, 275, 321, 330, 422, 198, 214, 134, 334, 477, 187, 680, 681, 501, 139, 199, 349, 301, 138, 571, 302, 475, 283, 276,/*567,*/530, /* 531, */ 421, 633, 673, 261, 684, 268, 299, 583, 478, 220, 569, 285, 75, 184,/*565,*/196, 529, 527, /* 533, */ 344, 197, 185, 140, 654,/*555,*/133, 73, 367, 65, 183, 64, 71, 420, 192, 186, 68, 72, 384, 132, 293, 70, 688, /*559,*/308, 195, 333, 317,/*563,*/74, 291, /* 66, */ 131, 392, 67, 189, 182, 181, 240, 599, 331, 193, "NULL"],
	[0, 405,/*564,*/652, 404, 78, 624,/*558,560,*/327, 402, 79, 611, 674, 661, 655, 394, 341, 340, 80, 243, 679,/*556,*/615, 141, 635, 573,/*562,*/267, 346, 83, 594, 616, 86, 82, 258, 482, 401,/*553,*/483, 660, 84, 81, 372, 87, 271, 400, 403, 85, 352, 581,/*554,*/77, 480, 481, 682, 338,/*566,*/294, 369, 295, 76, 393, "NULL"],
	[0, 662, 229, 227, 228, 246, 595, 266, 88, 269, 625, 614, 627, 322, 644, 568, 95, 94, 242, 486, 304, 362, 257, 423, 407, 408, 278, 523, 604, 484, 399, 638, 409, 89, 90, 381, 351, 377, 379, 129, 406, 606, 622, 388, 667, 665, 300, 91, 651, 92, 391, 93, 318, 485, "NULL"],
	[0, 230, 412, 188, 416, 142, 411, 365, 350, 640,  370, 270, 610, 634, 147, 685, 686, 417, 324, 591, 414, 262, 490, 256, 489, 641, /* 542, */ 335, 410, 418, 354, 495, 389, 151, 419, 216, 492, 493, 226, 152, /* 546, */ 103, 97, 218, 101, 371, 144, 597, 378, 639, 104, 628, 149, 250, 130, 248, 247, 653, 105, 150, 415, 143, 251, 148, 212, 494, 386, 677, 669, 629, 316, 678, /* 541, */ 297, 260, 488, 315, 146, 487, 96, 314, 491, 312, 336, 98, 292, 587, 656, 236, 145, 413, 237, 99, 385, 102, 100, 395, "NULL"]
];

function allCard() {
	var c = document.calcForm;
	c.A_weapon1_card1.length = 0;
	c.A_weapon1_card2.length = 0;
	c.A_weapon1_card3.length = 0;
	c.A_weapon1_card4.length = 0;
	/* if (n_Nitou) {
		c.A_weapon2_card1.length = 0;
		c.A_weapon2_card2.length = 0;
		c.A_weapon2_card3.length = 0;
		c.A_weapon2_card4.length = 0;
	} */
	c.A_head1_card.length = 0;
	c.A_head2_card.length = 0;
	c.A_left_card.length = 0;
	c.A_body_card.length = 0;
	c.A_shoulder_card.length = 0;
	c.A_shoes_card.length = 0;
	c.A_acces1_card.length = 0;
	c.A_acces2_card.length = 0;
	var all_cards = c.all_card.checked;
	var i = 0;
	if (!all_cards) {
		if(player.equip[EQI.HAND_R] == 0 || !(m_ForgedItems.includes(player.equip[EQI.HAND_R]))){
			for (i = 0; m_CardSort[0][i] != "NULL"; i++){
				c.A_weapon1_card1.options[i] = new Option(m_Card[m_CardSort[0][i]][2], m_Card[m_CardSort[0][i]][0]);
				c.A_weapon1_card2.options[i] = new Option(m_Card[m_CardSort[0][i]][2], m_Card[m_CardSort[0][i]][0]);
				c.A_weapon1_card3.options[i] = new Option(m_Card[m_CardSort[0][i]][2], m_Card[m_CardSort[0][i]][0]);
				c.A_weapon1_card4.options[i] = new Option(m_Card[m_CardSort[0][i]][2], m_Card[m_CardSort[0][i]][0]);
			}
			//for (i = 0; m_CardSort[1][i] != "NULL"; i++) {
				
			//}
		}else{
			c.A_weapon1_card1.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
			c.A_weapon1_card1.options[1] = new Option(m_Card[201][2], m_Card[201][0]);
			c.A_weapon1_card1.options[2] = new Option(m_Card[202][2], m_Card[202][0]);
			c.A_weapon1_card1.options[3] = new Option(m_Card[203][2], m_Card[203][0]);
			c.A_weapon1_card1.options[4] = new Option(m_Card[204][2], m_Card[204][0]);
			c.A_weapon1_card1.options[5] = new Option(m_Card[106][2], m_Card[106][0]);

			c.A_weapon1_card2.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
			c.A_weapon1_card2.options[1] = new Option(m_Card[106][2], m_Card[106][0]);
			c.A_weapon1_card3.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
			c.A_weapon1_card3.options[1] = new Option(m_Card[106][2], m_Card[106][0]);
			c.A_weapon1_card4.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
			c.A_weapon1_card4.options[1] = new Option("* Top10 ranked", 106);
		}
		
		/* if (n_Nitou) {
			if(player.equip[EQI.HAND_L] == 0 || !(m_ForgedItems.includes(player.equip[EQI.HAND_L]))){
				for (i = 0; m_CardSort[0][i] != "NULL"; i++){
					c.A_weapon2_card1.options[i] = new Option(m_Card[m_CardSort[0][i]][2], m_Card[m_CardSort[0][i]][0]);
					c.A_weapon2_card2.options[i] = new Option(m_Card[m_CardSort[0][i]][2], m_Card[m_CardSort[0][i]][0]);
					c.A_weapon2_card3.options[i] = new Option(m_Card[m_CardSort[0][i]][2], m_Card[m_CardSort[0][i]][0]);
					c.A_weapon2_card4.options[i] = new Option(m_Card[m_CardSort[0][i]][2], m_Card[m_CardSort[0][i]][0]);
				}
			}else{
				c.A_weapon2_card1.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
				c.A_weapon2_card1.options[1] = new Option(m_Card[201][2], m_Card[201][0]);
				c.A_weapon2_card1.options[2] = new Option(m_Card[202][2], m_Card[202][0]);
				c.A_weapon2_card1.options[3] = new Option(m_Card[203][2], m_Card[203][0]);
				c.A_weapon2_card1.options[4] = new Option(m_Card[204][2], m_Card[204][0]);
				c.A_weapon2_card1.options[5] = new Option(m_Card[106][2], m_Card[106][0]);

				c.A_weapon2_card2.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
				c.A_weapon2_card2.options[1] = new Option(m_Card[106][2], m_Card[106][0]);
				c.A_weapon2_card3.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
				c.A_weapon2_card3.options[1] = new Option(m_Card[106][2], m_Card[106][0]);
				c.A_weapon2_card4.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
				c.A_weapon2_card4.options[1] = new Option("* Top10 ranked", 106);
			}
		}
 */
		for (i = 0; m_CardSort[1][i] != "NULL"; i++) {
			c.A_head1_card.options[i] = new Option(m_Card[m_CardSort[1][i]][2], m_Card[m_CardSort[1][i]][0]);
		}
		for (i = 0; m_CardSort[2][i] != "NULL"; i++) {
			c.A_head2_card.options[i] = new Option(m_Card[m_CardSort[2][i]][2], m_Card[m_CardSort[2][i]][0]);
		}
		for (i = 0; m_CardSort[3][i] != "NULL"; i++)
			c.A_left_card.options[i] = new Option(m_Card[m_CardSort[3][i]][2], m_Card[m_CardSort[3][i]][0]);
		for (i = 0; m_CardSort[4][i] != "NULL"; i++)
			c.A_body_card.options[i] = new Option(m_Card[m_CardSort[4][i]][2], m_Card[m_CardSort[4][i]][0]);
		for (i = 0; m_CardSort[5][i] != "NULL"; i++)
			c.A_shoulder_card.options[i] = new Option(m_Card[m_CardSort[5][i]][2], m_Card[m_CardSort[5][i]][0]);
		for (i = 0; m_CardSort[6][i] != "NULL"; i++)
			c.A_shoes_card.options[i] = new Option(m_Card[m_CardSort[6][i]][2], m_Card[m_CardSort[6][i]][0]);
		for (i = 0; m_CardSort[7][i] != "NULL"; i++) {
			c.A_acces1_card.options[i] = new Option(m_Card[m_CardSort[7][i]][2], m_Card[m_CardSort[7][i]][0]);
			c.A_acces2_card.options[i] = new Option(m_Card[m_CardSort[7][i]][2], m_Card[m_CardSort[7][i]][0]);
		}
	} else {
		c.A_weapon1_card1.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_weapon1_card2.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_weapon1_card3.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_weapon1_card4.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		if (player.dual_wield) {
			c.A_weapon2_card1.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
			c.A_weapon2_card2.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
			c.A_weapon2_card3.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
			c.A_weapon2_card4.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		}
		c.A_head1_card.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_head2_card.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_left_card.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_body_card.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_shoulder_card.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_shoes_card.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_acces1_card.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		c.A_acces2_card.options[0] = new Option(m_Card[0][2], m_Card[0][0]);
		var n = 0;
		for (j = 0; j < 9; j++) {
			for (i = 1; m_CardSort[j][i] != "NULL"; i++) {
				if (j != 1) {
					n++;
					c.A_weapon1_card1.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
					if (player.dual_wield) {
						c.A_weapon2_card1.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
					}
				}
			}
		}
		n = 0;
		for (j = 1; j < 9; j++) {
			for (i = 1; m_CardSort[j][i] != "NULL"; i++) {
				n++;
				c.A_weapon1_card2.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_weapon1_card3.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_weapon1_card4.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				if (player.dual_wield) {
					c.A_weapon2_card2.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
					c.A_weapon2_card3.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
					c.A_weapon2_card4.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				}
				c.A_head1_card.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_head2_card.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_left_card.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_body_card.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_shoulder_card.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_shoes_card.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_acces1_card.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
				c.A_acces2_card.options[n] = new Option(m_Card[m_CardSort[j][i]][2], m_Card[m_CardSort[j][i]][0]);
			}
		}
		c.A_weapon1_card4.options[1] = new Option("* Top10 ranked", 106);
		if (player.dual_wield) {
			c.A_weapon2_card4.options[1] = new Option("* Top10 ranked", 106);
		}
	}
}


function Click_Card(value) {
	player.card[0] = 1 * c.A_weapon1_card1.value;
	player.card[1] = 1 * c.A_weapon1_card2.value;
	player.card[2] = 1 * c.A_weapon1_card3.value;
	player.card[3] = 1 * c.A_weapon1_card4.value;
	if (player.dual_wield) {
		player.card[4] = 1 * c.A_weapon2_card1.value;
		player.card[5] = 1 * c.A_weapon2_card2.value;
		player.card[6] = 1 * c.A_weapon2_card3.value;
		player.card[7] = 1 * c.A_weapon2_card4.value;
	} else {
		player.card[4] = 0;
		player.card[5] = 0;
		player.card[6] = 0;
		player.card[7] = 0;
	}
	player.card[8] = 1 * c.A_head1_card.value;
	player.card[9] = 1 * c.A_head2_card.value;
	player.card[10] = 1 * c.A_left_card.value;
	player.card[11] = 1 * c.A_body_card.value;
	player.card[12] = 1 * c.A_shoulder_card.value;
	player.card[13] = 1 * c.A_shoes_card.value;
	player.card[14] = 1 * c.A_acces1_card.value;
	player.card[15] = 1 * c.A_acces2_card.value;

	StAllCalc();
	Card(value);
	calc();
}

function Card(CBI) {
	if (typeof (CBI) == "undefined") return;
	//	n_itemSW=1*document.calcForm.ITEM_SW.checked;
	Item_or_Card = "Card";
	ItemCardNumberCheck = CBI;

	ActiveSkillSetPlus();

	if (n_itemSW == 0)
		return;
	for (i = 1; i <= 4; i++)
		myInnerHtml("ITEM" + i, "", 0);
	myInnerHtml("ITEM_W_LV", "", 0);
	myInnerHtml("ITEM_DATA", "", 0);
	myInnerHtml("ITEM_LV", "", 0);
	myInnerHtml("ITEM_WAIT", "", 0);

	if (CBI == 106) {
		myInnerHtml("ITEM_TITLE", "Very Strong or Top10", 0);
		myInnerHtml("B_SETUMEI", "1 Star Crumb adds +5 seeking damage.<BR>2 Star Crumb adds +10 seeking damage.<BR>3 Star Crumb adds +40 seeking damage.<BR>If is made by a Top 10 Blacksmith/Whitemisth Rank adds +10 seeking damage.", 0);
		return;
	}
	if (201 <= CBI && CBI <= 204) {
		myInnerHtml("ITEM_TITLE", m_Card[CBI][2], 0);
		myInnerHtml("B_SETUMEI", "Weapons " + v_Element[CBI - 200] + " to attribute", 0);
		return;
	}
	myInnerHtml("ITEM_TITLE", m_Card[CBI][2] + " Card", 0);

	CBIstr = "";
	for (i = 4; m_Card[CBI][i] != 0; i += 2)
		Item_Setumei(m_Card[CBI][i], m_Card[CBI][i + 1]);
	if (m_Card[CBI][3] != 0)
		CBIstr += m_Card[CBI][3] + "<BR>";

	var check = 0;
	for (var i = 4; m_Card[CBI][i] != 0; i += 2) {
		if (m_Card[CBI][i] == 90) {
			CBIstr += "<HR><B>When equipping " + SetCardName(m_Card[CBI][i + 1]);
			var w = w_SC[m_Card[CBI][i + 1]][0];
			while (m_Card[CBI][i + 2] != 0 && check == 0) {
				if (w == w_SE[m_Card[CBI][i + 3]][0]) {
					CBIstr += " or<BR>" + SetCardName(m_Card[CBI][i + 3]);
					i += 2;
				} else
					check = 1;
			}
			CBIstr += " at the same time:</B><BR>";
			check = 0;
			for (var j = 4; m_Card[w][j] != 0; j += 2)
				Item_Setumei(m_Card[w][j], m_Card[w][j + 1]);
			if (m_Card[w][3] != 0)
				CBIstr += m_Card[w][3] + "<BR>";
			CBIstr += "";
		}
	}
	myInnerHtml("B_SETUMEI", CBIstr, 0);
}

w_SC = [
	[429, 235, 306, "NULL"],
	[430, 349, 305, "NULL"],
	[431, 254, 259, 356, "NULL"],
	[432, 229, 280, 352, "NULL"],
	[433, 291, 234, "NULL"],
	[434, 322, 320, "NULL"],
	[435, 273, 98, "NULL"],
	[436, 274, 73, "NULL"],
	[437, 245, 40, "NULL"],
	[438, 9, 308, "NULL"],
	[439, 58, 276, "NULL"],
	[440, 50, 344, "NULL"],
	[441, 125, 370, 393, "NULL"],
	[442, 294, 10, "NULL"],
	[442, 341, 88, "NULL"],
	[442, 243, 7, "NULL"],
	[442, 372, 64, "NULL"],
	[442, 369, 68, "NULL"],
	[442, 338, 72, "NULL"],
	[448, 218, 212, "NULL"],
	[449, 248, 247, "NULL"],
	[450, 223, 317, 347, 354, 362, "NULL"],
	[451, 233, 295, 391, 395, 260, "NULL"],
	[452, 253, 383, 307, 301, 270, "NULL"],
	[453, 279, 224, 340, 351, 230, "NULL"],
	[454, 337, 358, 220, 346, 379, 350, "NULL"],
	[455, 326, 376, 281, 388, 216, "NULL"],
	[456, 190, 347, 354, 362, "NULL"],
	[457, 413, 113, 295, 391, 260, "NULL"],
	[458, 253, 383, 181, 270, "NULL"],
	[459, 279, 408, 224, 340, 230, "NULL"],
	[460, 337, 193, 346, 379, 350, "NULL"],
	[461, 326, 175, 281, 388, 104, "NULL"],
	[496, 485, 494, "NULL"],
	[570, 218, 569, "NULL"],
	[572, 212, 571, "NULL"],
	[620, 618, 613, "NULL"],
	[621, 618, 619, "NULL"],
	[649, 646, 648, "NULL"],
	[650, 647, 648, "NULL"],
];
SC_MAXnum = w_SC.length - 1;

for (var i = 0; i <= SC_MAXnum; i++) {
	for (var k = 1; w_SC[i][k] != "NULL"; k++) {
		for (var j = 4; m_Card[w_SC[i][k]][j] != 0; j += 2);
		m_Card[w_SC[i][k]][j] = 90;
		m_Card[w_SC[i][k]][j + 1] = i;
		m_Card[w_SC[i][k]][j + 2] = 0;
	}
}

function SetCardName(SENw) {
	var SENstr = "";
	for (var i = 0; i <= SC_MAXnum; i++) {
		if (i == SENw) {
			for (var j = 1; w_SC[i][j] != "NULL"; j++) {
				SENstr += "[" + m_Card[w_SC[i][j]][2] + " Card]";
				if (w_SC[i][j + 1] != "NULL")
					SENstr += "+";
			}
			return SENstr;
		}
	}
}

// format is [id, name, effect id, 0]
m_RandomOpt = [
	[0, "(no option)", 0, 0]
	, [1, "ATK [17]", 17, 0]
	, [2, "MATK [19]", 88, 0]
	, [3, "VIT [5]", 3, 0]
	, [4, "DEX [7]", 5, 0]
	, [5, "AGI [4]", 2, 0]
	, [6, "LUK [8]", 6, 0]
	, [7, "STR [3]", 1, 0]
	, [8, "INT [6]", 4, 0]
	, [9, "ASPD (%) [16]", 12, 0]
	, [10, "SP Cost Reduction [172]", 370, 0] 
	, [11, "CRIT [24]", 10, 0]
	, [12, "Critical Damage [164]", 70, 0]
	, [13, "Cast Time Reduction [170]", 73, 0] // use negative value here for reduction
	, [14, "After-cast Delay Reduction [171]", 74, 0]
	, [15, "FLEE [22]", 9, 0]
	, [16, "HIT [18]", 8, 0]
	, [17, "HP [1]", 13, 0]
	, [18, "SP [2]", 14, 0]
	, [19, "HP (%) [9]", 15, 0]
	, [20, "SP (%) [10]", 16, 0]
	, [21, "HP recovered per 10 secs [11]", 0, 0] // no actual effect
	, [22, "SP recovered per 10 secs [12]", 0, 0] // no actual effect
	, [23, "Perfect Dodge [23]", 11, 0]
	, [24, "DEF [20]", 18, 0]
	, [25, "MDEF [21]", 19, 0]
	, [26, "Healing Power [168]", 99, 0]
	, [27, "Freeze Resist [255]", 152, 0]
	, [28, "Stone Curse Resist [256]", 159, 0]
	, [29, "Neutral Element Resist [25]", 60, 0]
	, [30, "Water Element Resist [26]", 61, 0]
	, [31, "Earth Element Resist [27]", 62, 0]
	, [32, "Fire Element Resist [28]", 63, 0]
	, [33, "Wind Element Resist [29]", 64, 0]
	, [34, "Poison Element Resist [30]", 65, 0]
	, [35, "Holy Element Resist [31]", 66, 0]
	, [36, "Shadow Element Resist [32]", 67, 0]
	, [37, "Ghost Element Resist [33]", 68, 0]
	, [38, "Undead Element Resist [34]", 69, 0]
	, [39, "Formless Race Resist [87]", 50, 0]
	, [40, "Undead Race Resist [88]", 51, 0]
	, [41, "Brute Race Resist [89]", 52, 0]
	, [42, "Plant Race Resist [90]", 53, 0]
	, [43, "Insect Race Resist [91]", 54, 0]
	, [44, "Fish Race Resist [92]", 55, 0]
	, [45, "Demon Race Resist [93]", 56, 0]
	, [46, "Demi-Human Race Resist [94]", 57, 0]
	, [47, "Angel Race Resist [95]", 58, 0]
	, [48, "Dragon Race Resist [96]", 59, 0]
	, [49, "Small Size Resist [160]", 190, 0]
	, [50, "Medium Size Resist [161]", 191, 0]
	, [51, "Large Size Resist [162]", 192, 0]
	, [52, "Long Range Damage Resist [167]", 78, 0]
	, [53, "Boss Monster Resist [150]", 77, 0]
	, [54, "Small Physical Damage [157]", 27, 0]
	, [55, "Medium Physical Damage [158]", 28, 0]
	, [56, "Large Physical Damage [159]", 29, 0]
	, [57, "Long Range Physical Damage [166]", 25, 0]
	, [58, "Boss Physical Damage [148]", 26, 0]
	, [59, "Neutral Physical Damage [37]", 40, 0]
	, [60, "Water Physical Damage [39]", 41, 0]
	, [61, "Earth Physical Damage [41]", 42, 0]
	, [62, "Fire Physical Damage [43]", 43, 0]
	, [63, "Wind Physical Damage [45]", 44, 0]
	, [64, "Poison Physical Damage [47]", 45, 0]
	, [65, "Holy Physical Damage [49]", 46, 0]
	, [66, "Shadow Physical Damage [51]", 47, 0]
	, [67, "Ghost Physical Damage [53]", 48, 0]
	, [68, "Undead Element Physical Damage [55]", 49, 0]
	, [69, "Formless Physical Damage [97]", 30, 0]
	, [70, "Undead Race Physical Damage [98]", 31, 0]
	, [71, "Brute Physical Damage [99]", 32, 0]
	, [72, "Plant Physical Damage [100]", 33, 0]
	, [73, "Insect Physical Damage [101]", 34, 0]
	, [74, "Fish Physical Damage [102]", 35, 0]
	, [75, "Demon Physical Damage [103]", 36, 0]
	, [76, "Demi-Human Physical Damage [104]", 37, 0]
	, [77, "Angel Physical Damage [105]", 38, 0]
	, [78, "Dragon Physical Damage [106]", 39, 0]
	, [79, "Neutral MDEF Pierce [263]", 360, 0] 
	, [80, "Water MDEF Pierce [264]", 361, 0]
	, [81, "Earth MDEF Pierce [265]", 362, 0]
	, [82, "Fire MDEF Pierce [266]", 363, 0]
	, [83, "Wind MDEF Pierce [267]", 364, 0]
	, [84, "Poison MDEF Pierce [268]", 365, 0]
	, [85, "Holy MDEF Pierce [269]", 366, 0]
	, [86, "Shadow MDEF Pierce [270]", 367, 0]
	, [87, "Ghost MDEF Pierce [271]", 368, 0]
	, [88, "Undead Element MDEF Pierce [272]", 369, 0]
	, [89, "Formless MDEF Pierce [137]", 310, 0] 
	, [90, "Undead Race MDEF Pierce [138]", 311, 0]
	, [91, "Brute MDEF Pierce [139]", 312, 0]
	, [92, "Plant MDEF Pierce [140]", 313, 0]
	, [93, "Insect MDEF Pierce [141]", 314, 0]
	, [94, "Fish MDEF Pierce [142]", 315, 0]
	, [95, "Demon MDEF Pierce [143]", 316, 0]
	, [96, "Demi-Human MDEF Pierce [144]", 317, 0]
	, [97, "Angel MDEF Pierce [145]", 318, 0]
	, [98, "Dragon MDEF Pierce [146]", 319, 0]
	, [99, "Boss Magical Damage [152]", 353, 0] 
	, [100, "* All Monster MDEF Pierce", 295, 0]
	, [101, "Indestructible Weapon", 0, 0] // no actual effect
	, [102, "Normal DEF Pierce [153]", 291, 0] 
	, [103, "Boss DEF Pierce [154]", 292, 0] 
];

m_RandomOptMelee = [
	[0, 1, 7, 5, 3, 8, 4, 6, 11, 10, 9, "NULL"],
	[0, 13, 17, 18, 16, 12, 58, 54, 55, 56, "NULL"],
	[0, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, "NULL"],
];

m_RandomOptRanged = [
	[0, 1, 7, 5, 3, 8, 4, 6, 11, 10, 9, "NULL"],
	[0, 13, 17, 18, 16, 12, 58, 54, 55, 56, 57, "NULL"],
	[0, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, "NULL"],
];

m_RandomOptMagic = [
	[0, 2, 7, 5, 3, 8, 4, 6, 17, 18, "NULL"],
	[0, 13, 14, 19, 20, 9, 10, 99, "NULL"],
	[0, 100, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, "NULL"],
];

m_RandomOptSpecial = [
	[0, 1, 7, 5, 4, 16, 11, 12, 54, 55, 56, 58, "NULL"],
	[0, 2, 3, 8, 6, 9, 17, 18, 13, 14, 99, "NULL"],
	[0, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, "NULL"],
	[0, 100, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, "NULL"],
];

m_RandomOptForged = [
	[0, 1, 9, 11, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, "NULL"],
	[0, 17, 18, 16, 15, 13, 14, 10, 57, "NULL"],
	[0, 12, 54, 55, 56, 58, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, "NULL"],
	[0, 7, 5, 3, 8, 4, 6, "NULL"],
];

m_RandomOptCrimson = [
	[0, 17, 18, 24, 25, "NULL"],
	[0, 49, 50, 51, 53, 9, 10, 15, 16, 101, "NULL"]
];

m_RandomOptPickaxe = [
	[0, 1, 9, 7, 4, 3, 11, 10, "NULL"],
	[0, 1, 10, 12, 58, 54, 55, 56, 102, 103, "NULL"],
	[0, 1, 10, 102, 103, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, "NULL"]
];

m_RandomOptMeleeType = [1, 2, 3, 4, 5, 6, 7, 8, 11, 13];
m_RandomOptRangedType = [10, 14, 15, 16, 17, 18, 19, 20, 21];
m_RandomOptMagicType = [9];
m_RandomOptSpecialType = [12];
m_RandomOptSpecialWeapons = [395, 939, 93, 539, 1839, 1869, 1870, 1871, 1851, 2011, 2102, 2143, 2212];
m_RandomOptForgedWeapons = [2019, 2020, 2027, 2028, 2029, 2030, 2034, 2035, 2036, 2043, 2044, 2045, 2049, 2050, 2056, 2057, 2058, 2062, 2063, 2064];
m_RandomOptCrimsonWeapons = [2105, 2106, 2107, 2108, 2109, 2110, 2111, 2113, 2114, 2115, 2116, 2117, 2118, 2119, 2120, 2121, 2122, 2123, 2124];
m_RandomOptMineWorkerPickaxe = [2178];

function Click_RandOpt(){
	player.randopt[0] = 1 * c.A_weapon1_ropt1.value;
	player.randopt[1] = 1 * c.WEAP1_ROPT1.value;
	player.randopt[2] = 1 * c.A_weapon1_ropt2.value;
	player.randopt[3] = 1 * c.WEAP1_ROPT2.value;
	player.randopt[4] = 1 * c.A_weapon1_ropt3.value;
	player.randopt[5] = 1 * c.WEAP1_ROPT3.value;
	player.randopt[6] = 1 * c.A_weapon1_ropt4.value;
	player.randopt[7] = 1 * c.WEAP1_ROPT4.value;
	if (player.dual_wield) {
		player.randopt[8] = 1 * c.A_weapon2_ropt1.value;
		player.randopt[9] = 1 * c.WEAP2_ROPT1.value;
		player.randopt[10] = 1 * c.A_weapon2_ropt2.value;
		player.randopt[11] = 1 * c.WEAP2_ROPT2.value;
		player.randopt[12] = 1 * c.A_weapon2_ropt3.value;
		player.randopt[13] = 1 * c.WEAP2_ROPT3.value;
		player.randopt[14] = 1 * c.A_weapon2_ropt4.value;
		player.randopt[15] = 1 * c.WEAP2_ROPT4.value;
	} else {
		player.randopt[8] = 0;
		player.randopt[9] = 0;
		player.randopt[10] = 0;
		player.randopt[11] = 0;
		player.randopt[12] = 0;
		player.randopt[13] = 0;
		player.randopt[14] = 0;
		player.randopt[15] = 0;
	}
	player.randopt[16] = 1 * c.A_body_ropt1.value;
	player.randopt[17] = 1 * c.BODY_ROPT1.value;
	player.randopt[18] = 1 * c.A_body_ropt2.value;
	player.randopt[19] = 1 * c.BODY_ROPT2.value;
	player.randopt[20] = 1 * c.A_shoulder_ropt1.value;
	player.randopt[21] = 1 * c.SHOULDER_ROPT1.value;
	player.randopt[22] = 1 * c.A_shoulder_ropt2.value;
	player.randopt[23] = 1 * c.SHOULDER_ROPT2.value;
	player.randopt[24] = 1 * c.A_shoes_ropt1.value;
	player.randopt[25] = 1 * c.SHOES_ROPT1.value;
	player.randopt[26] = 1 * c.A_shoes_ropt2.value;
	player.randopt[27] = 1 * c.SHOES_ROPT2.value;
}

function RandOptWeapon1Reset(){
	myInnerHtml("nA_weapon1_ropt1", '<select name="A_weapon1_ropt1" onchange="Click_RandOpt(this[this.selectedIndex].value)|RandOptUpdate()" style="width: 218px; background-color: rgb(255, 204, 136);"></select> <input type="text" inputmode="numeric" style="width: 30px; height: 13px;" maxlength="3" onkeypress="return isNumeric(event)" onkeyup="RandOptUpdate()" name="WEAP1_ROPT1" value="0" class="center">', 0),
    myInnerHtml("nA_weapon1_ropt2", '<select name="A_weapon1_ropt2" onchange="Click_RandOpt(this[this.selectedIndex].value)|RandOptUpdate()" style="width: 218px; background-color: rgb(255, 204, 136);"></select> <input type="text" inputmode="numeric" style="width: 30px; height: 13px;" maxlength="3" onkeypress="return isNumeric(event)" onkeyup="RandOptUpdate()" name="WEAP1_ROPT2" value="0" class="center">', 0),
    myInnerHtml("nA_weapon1_ropt3", '<select name="A_weapon1_ropt3" onchange="Click_RandOpt(this[this.selectedIndex].value)|RandOptUpdate()" style="width: 218px; background-color: rgb(255, 204, 136);"></select> <input type="text" inputmode="numeric" style="width: 30px; height: 13px;" maxlength="3" onkeypress="return isNumeric(event)" onkeyup="RandOptUpdate()" name="WEAP1_ROPT3" value="0" class="center">', 0),
    myInnerHtml("nA_weapon1_ropt4", '<select name="A_weapon1_ropt4" onchange="Click_RandOpt(this[this.selectedIndex].value)|RandOptUpdate()" style="width: 218px; background-color: rgb(255, 204, 136);"></select> <input type="text" inputmode="numeric" style="width: 30px; height: 13px;" maxlength="3" onkeypress="return isNumeric(event)" onkeyup="RandOptUpdate()" name="WEAP1_ROPT4" value="0" class="center">', 0);
    c.A_weapon1_ropt1.options[0] = new Option(m_RandomOpt[0][1], m_RandomOpt[0][0]);
    c.A_weapon1_ropt2.options[0] = new Option(m_RandomOpt[0][1], m_RandomOpt[0][0]);
    c.A_weapon1_ropt3.options[0] = new Option(m_RandomOpt[0][1], m_RandomOpt[0][0]);
    c.A_weapon1_ropt4.options[0] = new Option(m_RandomOpt[0][1], m_RandomOpt[0][0]);
}

function RandOptWeapon2Reset(){
	myInnerHtml("nA_weapon2_ropt1", '<select name="A_weapon2_ropt1" onchange="Click_RandOpt(this[this.selectedIndex].value)|RandOptUpdate()" style="width: 218px; background-color: rgb(255, 204, 136);"></select> <input type="text" inputmode="numeric" style="width: 30px; height: 13px;" maxlength="3" onkeypress="return isNumeric(event)" onkeyup="RandOptUpdate()" name="WEAP2_ROPT1" value="0" class="center">', 0),
	myInnerHtml("nA_weapon2_ropt2", '<select name="A_weapon2_ropt2" onchange="Click_RandOpt(this[this.selectedIndex].value)|RandOptUpdate()" style="width: 218px; background-color: rgb(255, 204, 136);"></select> <input type="text" inputmode="numeric" style="width: 30px; height: 13px;" maxlength="3" onkeypress="return isNumeric(event)" onkeyup="RandOptUpdate()" name="WEAP2_ROPT2" value="0" class="center">', 0),
	myInnerHtml("nA_weapon2_ropt3", '<select name="A_weapon2_ropt3" onchange="Click_RandOpt(this[this.selectedIndex].value)|RandOptUpdate()" style="width: 218px; background-color: rgb(255, 204, 136);"></select> <input type="text" inputmode="numeric" style="width: 30px; height: 13px;" maxlength="3" onkeypress="return isNumeric(event)" onkeyup="RandOptUpdate()" name="WEAP2_ROPT3" value="0" class="center">', 0),
	myInnerHtml("nA_weapon2_ropt4", '<select name="A_weapon2_ropt4" onchange="Click_RandOpt(this[this.selectedIndex].value)|RandOptUpdate()" style="width: 218px; background-color: rgb(255, 204, 136);"></select> <input type="text" inputmode="numeric" style="width: 30px; height: 13px;" maxlength="3" onkeypress="return isNumeric(event)" onkeyup="RandOptUpdate()" name="WEAP2_ROPT4" value="0" class="center">', 0);
	c.A_weapon2_ropt1.options[0] = new Option(m_RandomOpt[0][1], m_RandomOpt[0][0]);
	c.A_weapon2_ropt2.options[0] = new Option(m_RandomOpt[0][1], m_RandomOpt[0][0]);
	c.A_weapon2_ropt3.options[0] = new Option(m_RandomOpt[0][1], m_RandomOpt[0][0]);
	c.A_weapon2_ropt4.options[0] = new Option(m_RandomOpt[0][1], m_RandomOpt[0][0]);
}

function RandOptReset(){
	c.A_weapon1_ropt1.value = 0;
	c.WEAP1_ROPT1.value = 0;
	c.A_weapon1_ropt2.value = 0;
	c.WEAP1_ROPT2.value = 0;
	c.A_weapon1_ropt3.value = 0;
	c.WEAP1_ROPT3.value = 0;
	c.A_weapon1_ropt4.value = 0;
	c.WEAP1_ROPT4.value = 0;
	if(player.dual_wield){
		c.A_weapon2_ropt1.value = 0;
		c.WEAP2_ROPT1.value = 0;
		c.A_weapon2_ropt2.value = 0;
		c.WEAP2_ROPT2.value = 0;
		c.A_weapon2_ropt3.value = 0;
		c.WEAP2_ROPT3.value = 0;
		c.A_weapon2_ropt4.value = 0;
		c.WEAP2_ROPT4.value = 0;
	}
	c.A_body_ropt1.value = 0;
	c.BODY_ROPT1.value = 0;
	c.A_body_ropt2.value = 0;
	c.BODY_ROPT2.value = 0;
	c.A_shoulder_ropt1.value = 0;
	c.SHOULDER_ROPT1.value = 0;
	c.A_shoulder_ropt2.value = 0;
	c.SHOULDER_ROPT2.value = 0;
	c.A_shoes_ropt1.value = 0;
	c.SHOES_ROPT1.value = 0;
	c.A_shoes_ropt2.value = 0;
	c.SHOES_ROPT2.value = 0;
}

m_Enchant = [
	[0, "(no enchant)", 0],
	[1, "10 ATK", 17, 10, 0],
	[2, "10 MATK", 88, 10, 0],
	[3, "150 HP", 13, 150, 0],
	[4, "30 SP", 14, 30, 0],
	[5, "3% ASPD", 12, 3, 0],
	[6, "3 CRIT & 3% Crit Damage", 10, 3, 70, 3, 0],
	[7, "5 FLEE", 9, 5, 0],
	[8, "5 HIT", 8, 5, 0],
	[9, "3 DEF & 3 MDEF", 18, 3, 19, 3, 0],
	[10, "5 CRIT", 10, 5, 0],
	[11, "10% Physical Damage to All Races", 30, 10, 31, 10, 32, 10, 33, 10, 34, 10, 35, 10, 36, 10, 37, 10, 38, 10, 39, 10, 0],
	[12, "5% Magic Damage to All Races", 170, 5, 171, 5, 172, 5, 173, 5, 174, 5, 175, 5, 176, 5, 177, 5, 178, 5, 179, 5, 0],
	[13, "5% ASPD", 12, 5, 0],
	[14, "5% Cast Time Reduction", 73, -5, 0],
	[15, "5% After-cast Delay Reduction", 74, 5, 0],
	[16, "5 CRIT & 5% Crit Damage", 10, 5, 70, 5, 0],
	[17, "1 STR", 1, 1, 0],
	[18, "1 DEX", 5, 1, 0],
	[19, "1 INT", 4, 1, 0],
];

m_HeadgearEnchant = [2190, 165, 211, 186];
m_ArmorEnchant = [2187];
m_GarmentEnchant = [2188];
m_ShoesEnchant = [2189];
m_WeaponEnchant = [2191, 2192, 2193, 2194, 2195, 2196, 2197, 2198];

m_AdventurerWeaponEnchant = [
	[0, 1, 2, 3, 4, 13, 11, 12, 16, 17, 18, 19, "NULL"],
	[0, 1, 2, 3, 4, 13, 11, 12, 16, 17, 18, 19, "NULL"]
];

m_AdventurerArcWandEnchant = [
	[0, 2, 3, 4, 13, 12, 14, 15, 17, 18, 19, "NULL"],
	[0, 2, 3, 4, 13, 12, 14, 15, 17, 18, 19, "NULL"]
];

function Click_Enchant(){
	player.enchant[0] = 1 * c.A_head_enchant.value;
	player.enchant[1] = 1 * c.A_body_enchant1.value;
	player.enchant[2] = 1 * c.A_body_enchant2.value;
	player.enchant[3] = 1 * c.A_shoulder_enchant1.value;
	player.enchant[4] = 1 * c.A_shoulder_enchant2.value;
	player.enchant[5] = 1 * c.A_shoes_enchant1.value;
	player.enchant[6] = 1 * c.A_shoes_enchant2.value;
	player.enchant[7] = 1 * c.A_weapon1_enchant1.value;
	player.enchant[8] = 1 * c.A_weapon1_enchant2.value;
	if (player.dual_wield) {
		player.enchant[9] = 1 * c.A_weapon2_enchant1.value;
		player.enchant[10] = 1 * c.A_weapon2_enchant2.value;
	} else {
		player.enchant[9] = 0;
		player.enchant[10] = 0;
	}
}

function LoadWeaponEnchants(id, oldWeaponId){
	if(id == 2192){
        if(oldWeaponId != 2192){
            c.A_weapon1_enchant1.length = 0;
            c.A_weapon1_enchant2.length = 0;
            for(i = 0; "NULL" != m_AdventurerArcWandEnchant[0][i]; i++){
                c.A_weapon1_enchant1.options[i] = new Option(m_Enchant[m_AdventurerArcWandEnchant[0][i]][1], m_Enchant[m_AdventurerArcWandEnchant[0][i]][0])
            }
            for(i = 0; "NULL" != m_AdventurerArcWandEnchant[1][i]; i++){
                c.A_weapon1_enchant2.options[i] = new Option(m_Enchant[m_AdventurerArcWandEnchant[1][i]][1], m_Enchant[m_AdventurerArcWandEnchant[1][i]][0])
            }
        }
	}else{
        if(!m_WeaponEnchant.includes(oldWeaponId)){
            for(i = 0; "NULL" != m_AdventurerWeaponEnchant[0][i]; i++){
                c.A_weapon1_enchant1.options[i] = new Option(m_Enchant[m_AdventurerWeaponEnchant[0][i]][1], m_Enchant[m_AdventurerWeaponEnchant[0][i]][0])
            }
            for(i = 0; "NULL" != m_AdventurerWeaponEnchant[1][i]; i++){
                c.A_weapon1_enchant2.options[i] = new Option(m_Enchant[m_AdventurerWeaponEnchant[1][i]][1], m_Enchant[m_AdventurerWeaponEnchant[1][i]][0])
            }
        }
	}
}

function loadNotes(monsterId){
    resetNotesStats();
	_ = '<tbody>',
	_ += '<tr><td colspan="12" class="subheader" style="background-color: rgb(68, 102, 102);">Notes:</td></tr>';
	for(i = 0; i < m_MonsterNotes.length; i++){
		if(monsterId == m_MonsterNotes[i][0]){
			_ += '<tr><td class="left" style="font-weight: bold">Monster:</td></tr>'
			for(j = 1; j < m_MonsterNotes[i].length && m_MonsterNotes[i][j] != "NULL"; j += 2){
				_ += Notes_Setumei(m_MonsterNotes[i][j], m_MonsterNotes[i][j + 1]);
				monster.notes[m_MonsterNotes[i][j]] = m_MonsterNotes[i][j + 1];
			}
			break
		}
	}
	for(i = 0; i < m_PlaceNotes.length; i++){
		if(c.ENEMY_SORT2.value == m_PlaceNotes[i][0]){
			_ += '<tr><td class="left" style="font-weight: bold">' + v_Place[m_PlaceNotes[i][0]] + ':</td></tr>'
			for(j = 1; j < m_PlaceNotes[i].length && m_PlaceNotes[i][j] != "NULL"; j += 2){
				_ += Notes_Setumei(m_PlaceNotes[i][j], m_PlaceNotes[i][j + 1]);
				monster.notes[m_PlaceNotes[i][j]] = m_PlaceNotes[i][j + 1];
			}
		}
	}
	_ += '</tbody>',
	myInnerHtml("monsterNotes", _, 0);
}

function NotesCalc(e, _){
	subDmg = 0;
	for(i = 0; i < m_MonsterNotes.length; i++){
		if(e == m_MonsterNotes[i][0]){
			for(j = 1; j < m_MonsterNotes[i].length && m_MonsterNotes[i][j] != "NULL"; j += 2){
				_ == m_MonsterNotes[i][j] && (subDmg = m_MonsterNotes[i][j + 1]);
			}
			break
		}
	}
	for(i = 0; i < m_PlaceNotes.length; i++){
		if(c.ENEMY_SORT2.value == m_PlaceNotes[i][0]){
			for(j = 1; j < m_PlaceNotes[i].length && m_PlaceNotes[i][j] != "NULL"; j += 2){
				_ == m_PlaceNotes[i][j] && (subDmg = m_PlaceNotes[i][j + 1]);
			}
		}
	}
	return subDmg;
}

function resetNotesStats(){
	monster.notes = [];
}

function Notes_Setumei(nC1, nC2){
	NotesStr = ""
	if(nC1 == 0)
		NotesStr += '<tr><td class="left">• Enchant Deadly Poison effect reduced by ' + nC2 + '%</td></tr>'
    if(nC1 == 1)
        NotesStr += '<tr><td class="left">• Cannot receive Melee Damage</td></tr>'
    if(nC1 == 2)
        NotesStr += '<tr><td class="left">• Cannot receive Ranged Damage</td></tr>'
    if(nC1 == 3)
        NotesStr += '<tr><td class="left">• Cannot receive Magic Damage</td></tr>'
    if(nC1 == 4)
        NotesStr += '<tr><td class="left">• Cannot receive Misc Damage</td></tr>'
	if(nC1 == 5)
		if(nC2 == 0)
			NotesStr += '<tr><td class="left">• Can only receive damage from single targeted skills</td></tr>'
		else
			NotesStr += '<tr><td class="left">• Can only receive damage from AOE skills</td></tr>'
	if(nC1 == 6)
		NotesStr += '<tr><td class="left">• Teleports when hit by any AOE skill</td></tr>'
	if (nC1 == 7)
		NotesStr += '<tr><td class="left">• Takes ' + nC2 + '% reduced damage</td></tr>'
	if(nC1 == 8)
		NotesStr += '<tr><td class="left">• Ignores DEF ignore and pierce</td></tr>'
	if(nC1 == 9)
		NotesStr += '<tr><td class="left">• Ignores MDEF ignore and pierce</td></tr>'
	if(nC1 == 10)
		NotesStr += '<tr><td class="left">• Ignores Ice Pick effect</td></tr>'
	if (5000 <= nC1 && nC1 <= 6999)
		NotesStr += '<tr><td class="left">• Receives -' + nC2 + '% damage from [' + skillName(m_Skill[nC1 - 5000][0]) + ']</td></tr>'
	return NotesStr;
}